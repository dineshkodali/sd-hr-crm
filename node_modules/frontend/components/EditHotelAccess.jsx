    ...
    <div className="p-4">
      ...
      <div className="space-y-2">
        <label className="flex items-center space-x-2">
          <input type="radio" name="visibility" value="public" checked={visibility === "public"} onChange={() => setVisibility("public")}  className="rounded-xl" />
          <span>Public</span>
        </label>
        <label className="flex items-center space-x-2">
          <input type="radio" name="visibility" value="private" checked={visibility === "private"} onChange={() => setVisibility("private")}  className="rounded-xl" />
          <span>Private</span>
        </label>
        <label className="flex items-center space-x-2">
          <input type="radio" name="visibility" value="select" checked={visibility === "select"} onChange={() => setVisibility("select")}  className="rounded-xl" />
          <span>Select People</span>
        </label>
      </div>

      {visibility === "select" && (
        <div className="mt-4 border p-4 bg-gray-50 rounded-xl">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium">Filter by Branch</label>
              <select value={selectedBranch} onChange={e => setSelectedBranch(e.target.value)} className="mt-1 block w-full border rounded-xl p-2">
            ...
            <div>
              ...
              <select value={selectedManager} onChange={e => setSelectedManager(e.target.value)} className="mt-1 block w-full border rounded-xl p-2">
          ...
          <div className="mt-4">
            {loading ? (
              ...
              <div className="p-6 bg-white rounded-xl text-gray-500">No users available to select.</div>
            ) : (
          ...
          <div className="mt-4 flex items-center space-x-2">
            <button className="px-4 py-2 bg-orange-600 text-white rounded-xl" onClick={handleConfirm} disabled={saving}>
      ...
      <div className="mt-4">
        ...
        <select className="mt-1 block w-full border rounded-xl p-2">
