...
export default function HSEIncidents({ user }) {
  ...
  const doDelete = async (id) => {
    ...
    try { await api.delete(`/api/hse/hse-incidents/${id}`); await refresh(); } catch (err) { alert('Delete failed'); }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="p-3 sm:p-4 md:p-6 w-[90%] max-w-[1800px] mx-auto">
        ...
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">HSE Incidents</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-500 text-sm mb-1">Investigating</div>
              <div className="text-2xl font-bold text-gray-900">{stats.investigating}</div>
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-500 text-sm mb-1">Critical / High</div>
                  ...
                  {loading ? (
                    ...
                    return (
                      <tr key={idx} className="hover:bg-gray-50 transition-colors">
                        ...
                        {visibleColumns.reference && (
                          <td className="py-4 px-4">
                            <span className="text-gray-700 font-medium">{r.reference || `HSE-${r.id || idx}`}</span>
                          </td>
                        )}
                        {visibleColumns.description && (
                          <td className="py-4 px-4">
                            <div>
                              ...
                              <div className="text-gray-500 text-xs mt-1">
                                {r.details || "HSE incident details and information."}
                              </div>
                        ...
                        {visibleColumns.priority && (
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-sm ${priorityStyle.text}`}>{r.severity || "Medium"}</span>
                            </div>
                          </td>
                        )}
                        {visibleColumns.status && (
                        ...
                        {visibleColumns.assigned && (
                          <td className="py-4 px-4">
                            {!r.assigned_investigator ? (
                              ...
                              <div className="flex items-center gap-2">
                                <div className={`w-8 h-8 rounded-full ${getAvatarColor(r.assigned_investigator)} flex items-center justify-center text-xs font-semibold`}>
                                  {getInitials(r.assigned_investigator)}
                                </div>
                                <span className="text-gray-900 text-sm">{r.assigned_investigator}</span>
                              </div>
                            )}
                          </td>
                        )}
                        {visibleColumns.date && (
                          <td className="py-4 px-4">
                            <span className="text-gray-700 text-sm">{formatDate(r.incident_date)}</span>
                          </td>
                        )}
                        {visibleColumns.actions && (
                          <td className="py-4 px-4">
                          ...
                          {statusItems.length === 0 ? (
                            ...
                            statusItems.map((incident) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs font-mono text-gray-500">{incident.reference || `INC-${incident.id}`}</span>
