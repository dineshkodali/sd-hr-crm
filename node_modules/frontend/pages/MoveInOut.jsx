    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="p-3 sm:p-4 md:p-6 w-[90%] max-w-[1800px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center gap-4 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-gray-200">
        ...
        {/* Content Panel */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 min-h-[400px] transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-slate-300">
          <div className="mb-6 flex items-center gap-3 border-b border-gray-200 px-6 pt-6">
            {[["ins", "Move-Ins"], ["outs", "Move-Outs"], ["active", "Active Residents"]].map(([tab, label]) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-3 px-1 text-sm font-medium border-b-2 transition-colors ${activeTab === tab ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>{label}</button>
            ))}
          </div>
                  ...
                  {showViewMenu && (
                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50">
                      <div className="p-4">
                        ...
                        <button onClick={() => setShowPropertyVisibility(!showPropertyVisibility)} className="w-full flex items-center justify-between px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-xl transition-colors">
                        ...
                        {showPropertyVisibility && (
                          <div className="mt-2 border-t border-gray-200 pt-3">
                            <div className="mb-3">
                              <div className="flex items-center justify-between mb-2"><span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Shown in view</span><button onClick={() => setVisibleColumns(ALL_COLUMNS.reduce((a, c) => ({ ...a, [c]: false }), {}))} className="text-xs text-teal-600 hover:text-teal-700 font-medium rounded-xl">Hide all</button></div>
                              <div className="space-y-1">{ALL_COLUMNS.filter(col => visibleColumns[col]).map(col => (<button key={col} onClick={() => setVisibleColumns({ ...visibleColumns, [col]: false })} className="w-full flex items-center justify-between px-2 py-1.5 text-sm text-gray-700 hover:bg-gray-50 rounded-xl transition-colors"><span className="capitalize">{col === 'serviceUser' ? 'Service User' : col === 'moveInDate' ? 'Move-In Date' : col}</span><Eye className="w-4 h-4 text-teal-600" /></button>))}</div>
                            </div>
                            {Object.values(visibleColumns).some(v => !v) && (
                              <div>
                                <div className="flex items-center justify-between mb-2"><span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Hidden in view</span><button onClick={() => setVisibleColumns(ALL_COLUMNS.reduce((a, c) => ({ ...a, [c]: true }), {}))} className="text-xs text-teal-600 hover:text-teal-700 font-medium rounded-xl">Show all</button></div>
                                <div className="space-y-1">{ALL_COLUMNS.filter(col => !visibleColumns[col]).map(col => (<button key={col} onClick={() => setVisibleColumns({ ...visibleColumns, [col]: true })} className="w-full flex items-center justify-between px-2 py-1.5 text-sm text-gray-400 hover:bg-gray-50 rounded-xl transition-colors"><span className="capitalize">{col === 'serviceUser' ? 'Service User' : col === 'moveInDate' ? 'Move-In Date' : col}</span></button>))}</div>
                              </div>
                            )}
                          </div>
                        )}
              ...
              activeMoveInsForDisplay.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {activeMoveInsForDisplay.map((r) => {
                    ...
                    return (
                      <div key={r.id} className="group bg-emerald-50/50 border border-emerald-100 rounded-xl p-5 flex items-center justify-between hover:border-emerald-200">
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => { setDetailRecord(r); setShowDetailModal(true); }} className="group relative p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50/50 rounded-xl hover:shadow-sm" title="View details"><Eye size={18} /></button>
                            {canUpdatePage && <button onClick={() => { setEditing(r); setShowModal(true); }} className="group relative p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50/50 rounded-xl hover:shadow-sm" title="Edit Record"><svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>}
                          </div>
                        </div>
                      </div>
                    );
              ...
              moveInsForDisplay.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {moveInsForDisplay.map((r) => {
                    ...
                    return (
                      <div key={r.id} className={`group bg-white border border-slate-200 rounded-xl p-5 flex items-center justify-between hover:border-slate-300 ${deletingIds.has(r.id) ? 'moveinout-card-deleting' : ''}`}>
                        <div className="flex items-center gap-5">
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => { setDetailRecord(r); setShowDetailModal(true); }} className="group relative p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50/50 rounded-xl hover:shadow-sm" title="View details"><IconEye size={18} /></button>
                            {canUpdatePage && <button onClick={() => { setEditing(r); setShowModal(true); }} className="group relative p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50/50 rounded-xl hover:shadow-sm" title="Edit Record"><IconEdit size={18} /></button>}
                            {canDeletePage && <button onClick={() => openDeleteConfirm(r.id, "move-in", "Delete Move-In", "Delete this move-in record? This action cannot be undone.")} className="group relative p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50/50 rounded-xl hover:shadow-sm" title="Delete Record" disabled={deleting}><IconTrash size={16} /></button>}
                          </div>
                        </div>
                      </div>
                    );
                    ...
                    return (
                      <div key={r.id} className={`group bg-white border border-slate-200 rounded-xl p-5 flex items-center justify-between hover:border-slate-300 ${deletingIds.has(r.id) ? 'moveinout-card-deleting' : ''}`}>
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          <div className="h-8 w-px bg-slate-100"></div>
                          <div className="flex items-center gap-1.5">
                            {canUpdatePage && <button onClick={() => { setEditing(r); setShowOutModal(true); }} className="group relative p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50/50 rounded-xl hover:shadow-sm" title="Edit Record"><IconEdit size={18} /></button>}
                            {canDeletePage && <button onClick={() => openDeleteConfirm(r.id, "move-out", "Delete Move-Out", "Delete this move-out record? This action cannot be undone.")} className="group relative p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50/50 rounded-xl hover:shadow-sm" title="Delete Record" disabled={deleting}><IconTrash size={18} /></button>}
                          </div>
                        </div>
                      </div>
                    );
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              ...
              <button onClick={closeExport} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500"><X size={18} /></button>
            </div>
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-3">
                <div className="text-sm font-medium text-gray-700">Columns</div>
                <div className="flex items-center gap-3 text-xs"><button onClick={() => setSelectedExportKeys(exportColumns.map((c) => c.key))} className="text-teal-600 hover:text-teal-700 font-medium rounded-xl">Select all</button><button onClick={() => setSelectedExportKeys([])} className="text-gray-600 hover:text-gray-700 font-medium rounded-xl">Clear</button></div>
              </div>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={closeExport} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">Cancel</button>
              <button onClick={runExport} className="px-4 py-2 text-sm font-medium text-white bg-teal-500 rounded-xl hover:bg-teal-600 transition-colors">Download</button>
            </div>
          </div>
        </div>
      )}
