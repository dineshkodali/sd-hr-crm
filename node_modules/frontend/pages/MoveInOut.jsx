...
if (typeof document !== 'undefined' && !document.getElementById(DELETE_STYLE_ID)) {
  ...
  style.textContent = `
    @keyframes moveInOutCardDelete {
      ...
      30%  { opacity: 0.6; transform: scale(1.04) rotate(-1.5deg); background: var(--bg-primary); }
    ...
    .form-select, .form-input, select, input, textarea {
      color: var(--text-primary) !important;
    ...
    .form-select:focus, .form-input:focus, select:focus, input:focus, textarea:focus {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="p-3 sm:p-4 md:p-6 w-[90%] max-w-[1800px] mx-auto">
        <div className="mb-6 flex items-start justify-between">
          <div>
            ...
            <h1 className="text-3xl font-black text-[var(--text-primary)] mt-1">Move-In/Out Dashboard</h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-[var(--bg-surface)] p-5 rounded-xl shadow-sm border border-[var(--border-color)] flex items-center gap-4 transition-all duration-200">
            ...
            <div className="flex-1 min-w-0"><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-0.5">Active Residents</div><div className="text-2xl font-black text-[var(--text-primary)] leading-none">{counts.active}</div><div className="text-xs text-[var(--text-secondary)] mt-1">Currently housed</div></div>
          </div>
          <div className="bg-[var(--bg-surface)] p-5 rounded-xl shadow-sm border border-[var(--border-color)] flex items-center gap-4 transition-all duration-200">
            ...
            <div className="flex-1 min-w-0"><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-0.5">Recent Move-Ins</div><div className="text-2xl font-black text-[var(--text-primary)] leading-none">{counts.moveIns}</div><div className="text-xs text-[var(--text-secondary)] mt-1">Last 30 days</div></div>
          </div>
          <div className="bg-[var(--bg-surface)] p-5 rounded-xl shadow-sm border border-[var(--border-color)] flex items-center gap-4 transition-all duration-200">
            ...
            <div className="flex-1 min-w-0"><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-0.5">Recent Move-Outs</div><div className="text-2xl font-black text-[var(--text-primary)] leading-none">{counts.moveOuts}</div><div className="text-xs text-[var(--text-secondary)] mt-1">Last 30 days</div></div>
          </div>
        </div>
        ...
        <div className="bg-[var(--bg-surface)] rounded-xl shadow-sm border border-[var(--border-color)] min-h-[400px] transition-all duration-200">
          <TabPills
            ...
            tabs={[
              { key: 'ins', label: 'Move-Ins' },
              { key: 'outs', label: 'Move-Outs' },
          ...
          <div className="p-6">
            <div className="mb-6">
              <div className="mb-4">
                <h3 className="text-lg font-bold text-[var(--text-primary)] mb-1">{activeTab === "ins" ? "Recent Move-Ins" : activeTab === "outs" ? "Recent Move-Outs" : "Active Residents"}</h3>
                <p className="text-sm font-medium text-[var(--text-secondary)]">{activeTab === "ins" ? "Service users who recently moved into accommodation" : activeTab === "outs" ? "Service users who recently moved out" : "Currently active residents"}</p>
              </div>

              <div className="flex items-center gap-3">
                {/* Filters Toggle */}
                <FiltersButton
                  activeCount={[filterProperty, filterStatus !== 'All Status' ? filterStatus : '', sortBy].filter(Boolean).length}
                  ...
                  {showViewMenu && (
                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50">
                      <div className="p-4">
                        ...
                        <button onClick={() => setShowPropertyVisibility(!showPropertyVisibility)} className="w-full flex items-center justify-between px-3 py-2 text-sm text-[var(--text-primary)] rounded-xl transition-colors hover:bg-[var(--bg-primary)]">
                          <span>Column visibility</span>
                          <div className="flex items-center gap-2"><span className="text-xs text-[var(--text-secondary)]">{Object.values(visibleColumns).filter(Boolean).length} shown</span><ChevronDown className={`w-4 h-4 transition-transform ${showPropertyVisibility ? 'rotate-180' : ''}`} /></div>
                        </button>
            ...
            {activeTab === "active" && (
              activeMoveInsForDisplay.length > 0 ? (
                    ...
                    return (
                      <div key={r.id} className="group bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl p-5 flex items-center justify-between hover:bg-[var(--bg-primary)]/60">
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <div className="text-[15px] font-extrabold text-[var(--text-primary)] leading-tight">{name}</div>
                            <div className="text-[13px] font-semibold text-[var(--text-secondary)] flex items-center gap-2 mt-1">
                              <span className="font-bold text-emerald-700">{r.property_name || "No Property"}</span>
                        ...
                        <div className="flex items-center gap-6">
                          <div className="hidden sm:flex flex-col items-end"><span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 border border-emerald-200"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 mr-2 animate-pulse"></span>Active Resident</span></div>
                          <div className="h-8 w-px bg-[var(--border-color)]"></div>
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => { setDetailRecord(r); setShowDetailModal(true); }} className="group relative p-2 text-[var(--text-secondary)] rounded-xl" title="View details"><Eye size={18} /></button>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center bg-[var(--bg-primary)] rounded-xl border border-dashed border-[var(--border-color)]">
                  <div className="bg-[var(--bg-surface)] p-4 rounded-full mb-4 shadow-sm"><Home className="text-[var(--text-secondary)]/40" size={32} /></div>
                  <div className="text-[var(--text-primary)] font-semibold text-lg">No active residents found</div>
                  <div className="text-[var(--text-secondary)] text-sm mt-1">Start by moving in a service user.</div>
                </div>
              )
            )}
                    ...
                    return (
                      <div key={r.id} className={`group bg-white border border-slate-200 rounded-xl p-5 flex items-center justify-between ${deletingIds.has(r.id) ? 'moveinout-card-deleting' : 'hover:bg-[var(--bg-primary)]/60'}`}>
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <div className="text-[var(--text-primary)] font-bold text-base">{name}</div>
                    ...
                    return (
                      <div key={r.id} className={`group bg-white border border-slate-200 rounded-xl p-5 flex items-center justify-between ${deletingIds.has(r.id) ? 'moveinout-card-deleting' : 'hover:bg-[var(--bg-primary)]/60'}`}>
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <div className="text-[var(--text-primary)] font-bold text-base">{name}</div>
                            <div className="text-sm text-[var(--text-secondary)] flex items-center gap-2 mt-0.5"><span>Moved out {formatted}</span></div>
                          </div>
                        </div>
                        <div className="flex items-center gap-6">
      ...
      <FiltersDrawer
        ...
        onClear={() => { setFilterProperty(''); setFilterStatus('All Status'); setSortBy(''); }}
      >
        <FilterField label="Property" icon={Filter} value={filterProperty} onChange={(e) => setFilterProperty(e.target.value)}>
      ...
      {showExportModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 transition-opacity">
          <div className="w-full max-w-2xl rounded-2xl bg-[var(--bg-surface)] shadow-2xl border border-[var(--border-color)] overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)] bg-[var(--bg-primary)]">
              <div><div className="text-lg font-bold text-[var(--text-primary)]">Download {exportFormat === 'pdf' ? 'PDF' : 'CSV'}</div><div className="text-xs text-[var(--text-secondary)] mt-0.5">Select the columns you want to include</div></div>
              <button onClick={closeExport} className="p-2 rounded-xl text-[var(--text-secondary)] hover:bg-[var(--bg-surface)] transition-colors"><X size={18} /></button>
            </div>
            <div className="px-6 py-6 bg-[var(--bg-surface)]">
              <div className="flex items-center justify-between mb-4">
                <div className="text-sm font-semibold text-[var(--text-primary)]">Columns</div>
                <div className="flex items-center gap-3 text-xs"><button onClick={() => setSelectedExportKeys(exportColumns.map((c) => c.key))} className="text-[var(--accent-primary)] font-bold hover:underline transition-all">Select all</button><button onClick={() => setSelectedExportKeys([])} className="text-[var(--text-secondary)] font-bold hover:underline transition-all">Clear</button></div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[45vh] overflow-auto pr-1">
                {exportColumns.map((col) => {
                  ...
                  return (
                    <label key={col.key} className={`flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 cursor-pointer ${checked ? 'border-[var(--accent-primary)] bg-[var(--accent-shadow)]' : 'border-[var(--border-color)] bg-[var(--bg-primary)] hover:border-[var(--accent-primary)]/30'}`}>
                      <input type="checkbox" checked={checked} onChange={(e) => { const isChecked = e.target.checked; setSelectedExportKeys((prev) => { const set = new Set(prev || []); if (isChecked) set.add(col.key); else set.delete(col.key); return Array.from(set); }); }} className="w-4 h-4 text-[var(--accent-primary)] rounded-md border-[var(--border-color)]" />
                      <span className={`text-sm font-medium ${checked ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>{col.header}</span>
                    </label>
                  );
                })}
              </div>
            ...
            <div className="px-6 py-4 border-t border-[var(--border-color)] bg-[var(--bg-primary)] flex justify-end gap-3">
              <button onClick={closeExport} className="px-5 py-2 text-sm font-bold text-[var(--text-secondary)] bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl transition-all hover:bg-[var(--bg-primary)]">Cancel</button>
              <button onClick={runExport} className="btn-primary rounded-xl">Download</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="fixed inset-0 z-60 flex items-start justify-center bg-slate-900/60 backdrop-blur-md p-4 pt-10 overflow-y-auto">
      <div className="bg-transparent w-full max-w-5xl">
        <div className="bg-[var(--bg-surface)] rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 border border-[var(--border-color)]">
          <div className="flex items-start justify-between p-6 border-b border-[var(--border-color)] bg-[var(--bg-primary)]">
            <div><h2 className="text-xl font-bold text-[var(--text-primary)]">Move-In/Out Details</h2><p className="text-sm text-[var(--text-secondary)] mt-1 font-medium">Detailed move-in & move-out information</p></div>
            <div className="flex items-center gap-3">
              <div>{movedOut ? (<span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-red-50 text-red-600 border border-red-100">Inactive</span>) : (<span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">Active resident</span>)}</div>
              <button onClick={() => onClose()} className="p-2 rounded-xl text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] transition-all"><X size={20} /></button>
            </div>
          </div>
          <div className="p-6 space-y-6">
            <div className="bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl p-6">
              <h4 className="text-lg font-bold text-[var(--text-primary)]">Move-In Details</h4>
              <p className="text-sm text-[var(--text-secondary)] mt-1">Checklist, notes and signature captured at move-in</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-1.2">Move-In Date</div><div className="mt-1 font-semibold text-[var(--text-primary)]">{get("move_in_date") !== "Not specified" ? fmt(get("move_in_date")) : "Not specified"}</div></div>
                <div><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-1.2">Notes</div><div className="mt-1 font-semibold text-[var(--text-primary)] italic">{r.notes || "No notes provided"}</div></div>
                <div className="md:col-span-2">
                  <div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-2.5">Move-In Checklist</div>
                  <div className="mt-2 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl p-4 text-sm shadow-inner">
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">{(MOVE_IN_CHECKLIST_ITEMS || []).map((label, i) => { const v = (r.checklist && (r.checklist[i] !== undefined ? r.checklist[i] : r.checklist[String(i)])) || false; return (<li key={i} className={`flex items-center gap-3 p-2 rounded-lg transition-all ${v ? 'bg-[var(--bg-surface)] shadow-sm border border-[var(--border-color)]' : 'opacity-40'}`}><div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${v ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400"}`}>{v ? "✓" : ""}</div><div className={`font-medium ${v ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>{label}</div></li>); })}</ul>
                  </div>
                </div>
                <div><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-1.2">Digital Signature</div><div className="mt-3">{r.signature ? (typeof r.signature === "string" && r.signature.startsWith("data:") ? <img src={r.signature} alt="signature" className="max-h-32 rounded-xl border border-[var(--border-color)] bg-white p-2 shadow-sm" /> : <span className="font-mono text-xs break-all text-[var(--text-secondary)] bg-[var(--bg-primary)] p-2 rounded-lg border border-[var(--border-color)]">{r.signature}</span>) : <span className="text-sm text-[var(--text-secondary)] italic">Not provided</span>}</div></div>
              </div>
            </div>
            <div className="bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-xl p-6">
              <h4 className="text-lg font-bold text-[var(--text-primary)]">Move-Out Details</h4>
              <p className="text-sm text-[var(--text-secondary)] mt-1">Checklist, notes and signature captured at move-out</p>
              {movedOut ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  <div><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-1.2">Move-Out Date</div><div className="mt-1 font-semibold text-[var(--text-primary)]">{movedOut.move_out_date ? fmt(movedOut.move_out_date) : movedOut.created_at ? fmt(movedOut.created_at) : "Not specified"}</div></div>
                  <div><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-1.2">Notes</div><div className="mt-1 font-semibold text-[var(--text-primary)] italic">{movedOut?.notes || "No notes provided"}</div></div>
                  <div className="md:col-span-2">
                    <div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-2.5">Move-Out Checklist</div>
                    <div className="mt-2 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl p-4 text-sm shadow-inner">
                      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">{(MOVE_OUT_CHECKLIST_ITEMS || []).map((label, i) => { const v = (movedOut && movedOut.checklist && (movedOut.checklist[i] !== undefined ? movedOut.checklist[i] : movedOut.checklist[String(i)])) || false; return (<li key={i} className={`flex items-center gap-3 p-2 rounded-lg transition-all ${v ? 'bg-[var(--bg-surface)] shadow-sm border border-[var(--border-color)]' : 'opacity-40'}`}><div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${v ? "bg-red-500 text-white" : "bg-slate-200 text-slate-400"}`}>{v ? "✓" : ""}</div><div className={`font-medium ${v ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>{label}</div></li>); })}</ul>
                    </div>
                  </div>
                  <div><div className="text-[10px] font-bold text-[var(--text-secondary)]/60 uppercase tracking-widest mb-1.2">Exit Signature</div><div className="mt-3">{movedOut.signature ? (typeof movedOut.signature === "string" && movedOut.signature.startsWith("data:") ? <img src={movedOut.signature} alt="move-out signature" className="max-h-32 rounded-xl border border-[var(--border-color)] bg-white p-2 shadow-sm" /> : <span className="font-mono text-xs break-all text-[var(--text-secondary)] bg-[var(--bg-primary)] p-2 rounded-lg border border-[var(--border-color)]">{movedOut.signature}</span>) : <span className="text-sm text-[var(--text-secondary)] italic">Not provided</span>}</div></div>
                </div>
              ) : (
                <div className="mt-6 py-8 px-4 bg-[var(--bg-primary)] border border-dashed border-[var(--border-color)] rounded-xl text-center">
                  <p className="text-sm text-[var(--text-secondary)] font-medium">Resident is still active. No move-out data recorded.</p>
                </div>
              )}
            </div>
          </div>
...
function MoveInModal({
  ...
  recent = [], moveOuts = [], activeResidents = [], onClose = () => { }, onCreate = () => { },
  ...
  const eligibleResidents = useMemo(() => {
    // A resident is eligible if they are NOT in the activeResidents list
    const activeResidentIds = new Set(activeResidents.map(r => String(r.id).toLowerCase()));

    return (Array.isArray(serviceUsers) ? serviceUsers : [])
      .filter((su) => su?.id)
      .filter((su) => {
        const id = String(su.id).toLowerCase();
        // If we are editing, the current resident is always eligible
        if (initialRecord && (String(initialRecord.service_user_id) === id || String(initialRecord.serviceUserId) === id)) return true;
        // Otherwise, they must not be an active resident
        return !activeResidentIds.has(id);
      })
      .sort((a, b) => String(a?.name || '').localeCompare(String(b?.name || ''), undefined, { sensitivity: 'base' }));
  }, [serviceUsers, activeResidents, initialRecord]);
  ...
  function handleChange(field, value) {
    if (field === "serviceUserId") {
      ...
      const nextRoomId = su?.room_id ? String(su.room_id) : (su?.roomId ? String(su.roomId) : "");
      const nextBedspaceId = su?.bedspace_id ? String(su.bedspace_id) : (su?.bedspaceId ? String(su.bedspaceId) : "");
      ...
      setForm((f) => ({
        ...
        roomId: nextRoomId,
        bedspaceId: nextBedspaceId
      }));

      // Trigger data fetching for the new property/room
      if (nextPropertyId) {
        fetchRooms(nextPropertyId).then(() => {
          if (nextRoomId) fetchBedspaces(nextPropertyId, nextRoomId);
        });
      }
      return;
    }
  ...
  return createPortal(
    <div className="modal-overlay">
      <div className="modal-container h-[73vh] bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
        <div className="modal-header bg-[var(--bg-primary)] border-b border-[var(--border-color)] px-6 py-4">
          <div>
            <h2 className="modal-title text-xl font-bold text-[var(--text-primary)]">{initialRecord ? "Edit Move-In" : "Process Move-In"}</h2>
            <p className="modal-subtitle text-sm text-[var(--text-secondary)] mt-1 font-medium">{initialRecord ? "Update the move-in record" : "Complete the move-in checklist for a new resident"}</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl text-[var(--text-secondary)] hover:bg-[var(--bg-surface)] transition-all">
            <X size={20} />
          </button>
        </div>
          ...
          <form id="moveInForm" onSubmit={handleSubmit} className="form-section">
            <div className="form-grid-2">
              <div className="form-group">
                <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Resident <span className="text-red-500">*</span></label>
                <select
                ...
                {!initialRecord && eligibleResidents.length === 0 && (
                  <div className="text-[10px] text-[var(--text-secondary)] mt-1.5 font-medium">No eligible residents available.</div>
                )}
              </div>

              <div className="form-group">
                <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Property <span className="text-red-500">*</span></label>
                <select className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all cursor-pointer" required value={form.propertyId} onChange={(e) => handleChange("propertyId", e.target.value)}>
            ...
            <div className="form-grid-2">
              <div className="form-group">
                <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Room <span className="text-red-500">*</span></label>
                <select
                  className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all cursor-pointer"
                  required
                  value={form.roomId}
              ...
              <div className="form-group">
                <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Bedspace <span className="text-red-500">*</span></label>
                <select
                  className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all cursor-pointer"
                  required
            ...
            <div className="form-group">
              <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Move-In Date <span className="text-red-500">*</span></label>
              <input type="date" className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all" required value={form.moveInDate} onChange={(e) => handleChange("moveInDate", e.target.value)} />
            </div>

            <div className="form-group">
              <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-3 block">Move-In Checklist <span className="text-red-500">*</span></label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {checklistItems.map((t, i) => (
                  <label key={i} className={`flex items-start gap-3 p-3 rounded-xl border transition-all duration-200 cursor-pointer ${form.checklist[i] ? "border-[var(--accent-primary)] bg-[var(--accent-shadow)]" : "border-[var(--border-color)] bg-[var(--bg-primary)] hover:border-[var(--accent-primary)]/30"}`}>
                    <div className="pt-0.5">
                      <input type="checkbox" className="w-4 h-4 text-[var(--accent-primary)] rounded-md border-[var(--border-color)] focus:ring-[var(--accent-shadow)]" checked={!!form.checklist[i]} onChange={() => toggleItem(i)} />
                    </div>
                    <span className={`text-sm font-medium ${form.checklist[i] ? "text-[var(--text-primary)]" : "text-[var(--text-secondary)]"}`}>{t}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="form-group">
              <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Additional Notes <span className="text-red-500">*</span></label>
              <textarea className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all h-auto resize-none" rows={3} required value={form.notes} onChange={(e) => handleChange("notes", e.target.value)} placeholder="Any additional observations or comments..." />
            </div>
          </form>
        </div>
      ...
      <div className="modal-container">
        ...
        <div className="modal-content">
          <form id="moveOutForm" onSubmit={handleSubmit} className="form-section">
            <div className="form-grid-2">
              <div className="form-group">
                ...
                <select
                  required
                  disabled={!!initialRecord}
                  className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                  value={form.serviceUserId}
                  onChange={(e) => {
              ...
              <div className="form-group">
                <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Move-Out Date <span className="text-red-500">*</span></label>
                <input type="date" className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all" required value={form.moveOutDate} onChange={(e) => setForm({ ...form, moveOutDate: e.target.value })} />
              </div>
            </div>

            <div className="form-group">
              <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-3 block">Move-Out Checklist <span className="text-red-500">*</span></label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {checklistItems.map((t, i) => (
                  <label key={i} className={`flex items-start gap-3 p-3 rounded-xl border transition-all duration-200 cursor-pointer ${form.checklist[i] ? "border-[var(--accent-primary)] bg-[var(--accent-shadow)]" : "border-[var(--border-color)] bg-[var(--bg-primary)] hover:border-[var(--accent-primary)]/30"}`}>
                    <div className="pt-0.5">
            ...
            <div className="form-group">
              <label className="form-label text-sm font-bold text-[var(--text-primary)] mb-2 block">Notes <span className="text-red-500">*</span></label>
              <textarea className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl px-4 py-3 text-sm text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:ring-2 focus:ring-[var(--accent-shadow)] outline-none transition-all h-auto resize-none" rows={3} required value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Final remarks..." />
            </div>
          </form>
        </div>
