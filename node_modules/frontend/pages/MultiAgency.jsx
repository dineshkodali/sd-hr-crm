...
if (typeof document !== 'undefined' && !document.getElementById(DELETE_STYLE_ID)) {
    ...
    style.textContent = `
      ...
      @keyframes multiAgencyCardDelete {
      ...
      tr.multiagency-deleting {
        animation: multiAgencySlideOut 0.45s cubic-bezier(0.4,0,1,1) forwards;
        overflow: hidden;
        pointer-events: none;
      }
      .multiagency-card-deleting {
        animation: multiAgencyCardDelete 0.45s cubic-bezier(0.4,0,1,1) forwards;
        pointer-events: none;
      }
    `;
...
export default function MultiAgency({ user }) {
        ...
        const fetchAvailableColumns = async () => {
            try {
                ...
                const standardCols = ['id', 'reference', 'created_at', 'updated_at', 'created_by', 'updated_by',
                    ...
                    'assigned_to', 'reported_by', 'scheduled_date', 'status', 'attachments'];
    ...
    const refreshRecords = async () => {
        try {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    const r = await api.get('/api/safeguarding/multi-agency?limit=500');
                    setRecords(Array.isArray(r?.data) ? r.data : (r?.data?.rows ?? r?.data ?? []));
                    return;
                } catch (e) {
                    if (!isTransientNetworkError(e) || attempt === 2) throw e;
                    await sleep(attempt === 0 ? 400 : attempt === 1 ? 1200 : 2500);
                }
            }
        } catch (err) {
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                if (mounted) setHotelsLoading(false);
            }
        }
        load();
    ...
    const handleCloseModal = () => {
        ...
        _openGallery(items, "Multi-Agency Documents", "/api/multi-agency/attachments");
    };
    ...
    const removeAttachment = async (attachmentId) => {
        ...
        try {
            setSubmitting(true);
            await api.delete(`/api/safeguarding/multi-agency/attachments/${encodeURIComponent(String(attachmentId))}`).catch(() => null);
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            if (selectedPhotos.length > 0) {
                ...
                if (modalMode === 'create') {
                    await api.post('/api/safeguarding/multi-agency', fd, { headers: { 'Content-Type': 'multipart/form-data' } });
                } else {
                    await api.patch(`/api/safeguarding/multi-agency/${selectedRecord?.id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
                }
            } else {
                if (modalMode === 'create') {
                    await api.post('/api/safeguarding/multi-agency', payload);
                } else {
                    await api.patch(`/api/safeguarding/multi-agency/${selectedRecord?.id}`, payload);
                }
            }
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                try {
                    ...
                    await api.delete(`/api/safeguarding/multi-agency/${id}`).catch(() => null);
                    await refreshRecords();
                } catch (err) {
            ...
            <div className="p-3 sm:p-4 md:p-6">
                ...
                <div className="bg-white rounded-2xl shadow-2xs border border-slate-200 p-6 md:p-8 mb-6">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            <Breadcrumbs items={[{ label: 'Safeguarding', path: '/admin/safeguarding-referrals' }, { label: 'Multi-Agency', path: '/admin/multi-agency' }]} />
                            <h1 className="text-3xl font-black text-slate-900 mt-1">Multi-Agency Dashboard</h1>
                            <p className="text-xs font-semibold text-slate-400 mt-0.5">Manage multi-agency records and inter-agency collaboration</p>
                        </div>
                        {hasCreate && (
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="bg-white">
                                    {recordsLoading ? (
                                        ...
                                        return (
                                            <tr key={idx} className={`transition-all ${isDeleting ? 'multiagency-deleting' : 'hover:bg-[var(--bg-primary)]/60'}`}>
                                                {visibleColumns.checkbox && (
                                                ...
                                                {visibleColumns.reference && (
                                                    <td className="py-5 px-6">
                                                        <span className="text-slate-900 font-semibold text-sm whitespace-nowrap">{rec.reference || `MA-${rec.id || idx}`}</span>
                                                    </td>
                                                )}
                                                {visibleColumns.description && (
                                                    <td className="py-5 px-6">
                                                ...
                                                <div className="p-3 space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
                                                    {statusItems.length === 0 ? (
                                                        <div className="text-center py-8 px-4">
                                                            <Network className="w-10 h-10 mx-auto mb-2 text-[var(--text-secondary)]" />
                                                        ...
                                                        statusItems.map((record) => {
                                                            ...
                                                            return (
                                                                <div
                                                                    key={record.id}
                                                                    className={`bg-[var(--bg-surface)] rounded-xl p-4 shadow-sm border border-[var(--border-color)] transition-all cursor-pointer ${isDeleting ? 'multiagency-card-deleting' : ''}`}
                                                                    onClick={() => { setSelectedRecord(record); setModalMode('view'); setShowModal(true); }}
                                                                >
                                                                    <div className="flex items-center justify-between mb-2">
                                                                        <span className="text-xs font-mono text-[var(--text-secondary)]">{record.reference || `MA-${record.id}`}</span>
                ...
                <div className="modal-overlay">
                    <div className="modal-container h-[70vh]">
                        ...
                        <div className="modal-header">
                            <div>
                                <h2 className="modal-title">
                                    {modalMode === 'create' ? "New Multi-Agency Referral" : modalMode === 'edit' ? "Edit Referral" : "Referral Details"}
                                </h2>
                                <p className="modal-subtitle">
                                    {modalMode === 'view' ? 'View referral information' : 'Enter referral details'}
                                </p>
                            </div>
                        ...
                        {modalMode === 'view' ? (
                            ...
                            <>
                                <form id="multi-agency-form" onSubmit={submit} className="modal-content form-section">
