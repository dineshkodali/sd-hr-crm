...
export default function HSETraining({ user }) {
  ...
  const submit = async (e) => { e.preventDefault(); setSubmitting(true); setError(null); try { if (mode === 'create') await api.post('/api/hse/training', formData); else await api.patch(`/api/hse/training/${selected?.id}`, formData); await refresh(); closeModal(); } catch (err) { setError(err?.response?.data?.message || err?.message || 'Failed'); } finally { setSubmitting(false); } };

  const doDelete = async (id) => { if (!confirm('Are you sure you want to delete this record?')) return; try { await api.delete(`/api/hse/training/${id}`); await refresh(); } catch (err) { alert('Delete failed'); } };
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="p-3 sm:p-4 md:p-6 w-[90%] max-w-[1800px] mx-auto">
        ...
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">HSE Training</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            <div className="bg-red-100 text-red-600 h-14 w-14 rounded-full flex items-center justify-center shrink-0">
              <GraduationCap className="w-7 h-7" />
            </div>
            <div className="flex-1 min-w-0">
                  ...
                  {loading ? (
                    ...
                    return (
                      <tr key={idx} className="hover:bg-gray-50 transition-colors">
                        ...
                        {visibleColumns.reference && (
                          <td className="py-4 px-4">
                            <span className="text-gray-700 font-medium">{r.reference || `TRN-${r.id || idx}`}</span>
                          </td>
                        )}
                        {visibleColumns.description && (
                          <td className="py-4 px-4">
                            <div>
                              ...
                              <div className="text-gray-500 text-xs mt-1">
                                {r.description || "Training description and information."}
                              </div>
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <div className="flex items-center gap-3">
                <h3 className="text-lg font-bold text-gray-900">
                  {mode === 'create' ? "New HSE Training" : mode === 'edit' ? "Edit Training" : "View Training"}
                </h3>
                {mode === 'view' && (
            ...
            {mode === 'view' ? (
              <div className="p-6">
                ...
                <div className="mb-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-md text-sm text-slate-600 border border-slate-100 min-h-20">
