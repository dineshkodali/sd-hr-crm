...
export default function Bookings({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ first_name: '', last_name: '', date_of_birth: '', nationality: '', home_office_reference: '', property_id: '', room_id: '', check_in_date: '', vulnerabilities: '', medical_conditions: '', dietary_requirements: '' });
      setShowModal(false);
      setRooms([]);
      await loadData();
  ...
  ), [error, formData, handlePropertyChange, properties, rooms]);
      ...
      <div className="p-3 sm:p-4 md:p-6 w-[90%] max-w-[1800px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-6">
          {[
            ...
            <div key={label} onClick={onClick} className={`bg-white rounded-xl p-5 shadow-sm border border-gray-100 flex items-center gap-4 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-gray-200 ${extra ?? ''}`}>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-2xl rounded-xl bg-white shadow-2xl border border-gray-200 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                ...
                <button onClick={() => { setShowExportModal(false); setExportFormat(null); }} className="p-2 rounded-xl hover:bg-gray-100 text-gray-500"><X className="w-5 h-5" /></button>
              </div>
              <div className="px-5 py-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="text-sm font-medium text-gray-700">Columns</div>
                  <div className="flex gap-3 text-xs">
                    <button onClick={() => setSelectedExportKeys(exportColumns.map(c => c.key))} className="text-teal-600 hover:text-teal-700 font-medium">Select all</button>
                    <button onClick={() => setSelectedExportKeys([])} className="text-gray-600 hover:text-gray-700 font-medium">Clear</button>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[45vh] overflow-auto">
                  {exportColumns.map(col => (
                    <label key={col.key} className="flex items-center gap-2 p-2 rounded-xl border border-gray-100 hover:bg-gray-50">
        ...
        {/* Main Table Card */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-gray-200">
          ...
          <div className="mb-6 flex items-center gap-3 border-b border-gray-200">
            {[
              ...
              <button key={key} onClick={() => setActiveTab(key)} className={`pb-3 px-1 text-sm font-medium border-b-2 transition-colors ${activeTab === key ? 'border-teal-500 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>{label}</button>
            ))}
          </div>
                        ...
                        <div className="mb-3 pb-3 border-b border-gray-200">
                          ...
                          <div className="flex gap-2">
                            {[['table', <Columns className="w-4 h-4" />, 'Table'], ['board', <ClipboardList className="w-4 h-4" />, 'Board']].map(([mode, icon, label]) => (
                              <button key={mode} onClick={() => setViewMode(mode)} className={`flex-1 px-3 py-2 rounded-xl font-medium text-sm transition-colors flex items-center justify-center gap-2 ${viewMode === mode ? 'bg-teal-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{icon}<span>{label}</span></button>
                            ))}
                          </div>
                        </div>
                        {viewMode === 'table' && (
                          <>
                            <button onClick={() => setShowColumnVisibility(!showColumnVisibility)} className="w-full flex items-center justify-between px-2 py-2 text-sm text-gray-700 hover:bg-gray-50 rounded-xl">
                              ...
                              <div className="mt-2 border-t border-gray-200 pt-3">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <div className="text-xs font-medium flex gap-2">
                                    <button onClick={() => setVisibleColumns(ALL_COLUMNS.reduce((a, c) => ({ ...a, [c]: true }), {}))} className="text-teal-600 hover:text-teal-700">Show all</button>
                                    <span className="text-gray-300">|</span>
                                    <button onClick={() => setVisibleColumns(ALL_COLUMNS.reduce((a, c) => ({ ...a, [c]: false }), {}))} className="text-teal-600 hover:text-teal-700">Hide all</button>
                                  </div>
                                </div>
                                <div className="max-h-72 overflow-y-auto space-y-2">
                                  {ALL_COLUMNS.map(col => (
                                    <button key={col} type="button" onClick={() => setVisibleColumns(p => ({ ...p, [col]: !p[col] }))} className="w-full flex items-center justify-between px-3 py-2 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 transition-colors">
                ...
                <tbody className="divide-y divide-gray-100">
                  {isLoading ? (
                    ...
                    filteredBookings.map(booking => {
                      ...
                      return (
                        <tr key={booking.id} className={`hover:bg-gray-50 transition-colors ${isDeleting ? 'booking-deleting' : ''}`}>
                          ...
                          {visibleColumns.actions && (
                            <td className="py-3 px-4 text-center">
                              <div className="flex items-center justify-center gap-1.5">
                                {isPending && (
                                  <>
                                    <button onClick={() => handleApprove(booking)} className="group p-2 text-gray-400 hover:text-green-600 hover:bg-green-50/50 rounded-xl" title="Approve"><Check className="w-4 h-4 transition-transform group-hover:scale-110" /></button>
                                    <button onClick={() => handleReject(booking)} className="group p-2 text-gray-400 hover:text-red-600 hover:bg-red-50/50 rounded-xl" title="Reject"><X className="w-4 h-4 transition-transform group-hover:scale-110" /></button>
                                    <div className="w-px h-4 bg-gray-200 mx-1"></div>
                                  </>
                                )}
                                <button onClick={() => handleView(booking)} className="group p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50/50 rounded-xl" title="View"><Eye className="w-4 h-4 transition-transform group-hover:scale-110" /></button>
                                <button onClick={() => handleEdit(booking)} className="group p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50/50 rounded-xl" title="Edit"><Edit className="w-4 h-4 transition-transform group-hover:scale-110" /></button>
                                <button onClick={() => handleDelete(booking)} className="group p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50/50 rounded-xl" title="Delete"><Trash2 className="w-4 h-4 transition-transform group-hover:scale-110" /></button>
                              </div>
                            </td>
                          )}
                        </tr>
                  ...
                  return (
                    <div key={booking.id} className={`bg-white border border-gray-200 rounded-xl p-4 transition-all ${isDeleting ? 'board-card-deleting' : ''}`}>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-100 flex gap-2">
                        {isPending ? (
                          <>
                            <button onClick={() => handleApprove(booking)} className="flex-1 py-1.5 text-center text-sm bg-green-50 text-green-700 hover:bg-green-100 rounded-xl font-medium">Approve</button>
                            <button onClick={() => handleReject(booking)} className="flex-1 py-1.5 text-center text-sm bg-red-50 text-red-700 hover:bg-red-100 rounded-xl font-medium">Reject</button>
                          </>
                        ) : (
                          <div className="flex w-full gap-1.5">
                            <button onClick={() => handleView(booking)} className="flex-1 text-center text-sm text-teal-600 hover:text-teal-700 font-medium rounded-xl py-1.5 hover:bg-teal-50">View</button>
                            <button onClick={() => handleEdit(booking)} className="flex-1 text-center text-sm text-emerald-600 hover:text-emerald-700 font-medium rounded-xl py-1.5 hover:bg-emerald-50">Edit</button>
                            <button onClick={() => handleDelete(booking)} className="flex-1 text-center text-sm text-rose-600 hover:text-rose-700 font-medium rounded-xl py-1.5 hover:bg-rose-50">Delete</button>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <div className="modal-overlay">
          <div className="modal-container h-[70vh]">
            ...
            <div className="modal-footer">
              <button type="button" onClick={() => setShowModal(false)} disabled={submitting} className="px-3 py-2.5 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 text-sm font-medium">Cancel</button>
              <button type="submit" form="booking-form" disabled={submitting} className="px-3 py-2.5 bg-teal-500 text-white rounded-xl hover:bg-teal-600 text-sm font-medium">{submitting ? 'Creating…' : 'Create Booking'}</button>
            </div>
          </div>
        </div>,
        document.body
      ...
      {showEditModal && selectedBooking && createPortal(
        <div className="modal-overlay">
          <div className="modal-container h-[70vh]">
            ...
            <form id="edit-booking-form" onSubmit={async (e) => {
              ...
              try {
                ...
                if (selectedBooking.service_user_id) {
                  await api.put(`/api/su/users/${selectedBooking.service_user_id}`, {
                    first_name: formData.first_name, last_name: formData.last_name,
            ...
            <div className="modal-footer">
              <button type="button" onClick={() => setShowEditModal(false)} disabled={submitting} className="px-3 py-2.5 border border-gray-300 rounded-xl text-gray-700 hover:bg-gray-50 text-sm font-medium">Cancel</button>
              <button type="submit" form="edit-booking-form" disabled={submitting} className="px-3 py-2.5 bg-teal-500 text-white rounded-xl hover:bg-teal-600 text-sm font-medium">{submitting ? 'Updating…' : 'Update Booking'}</button>
            </div>
          </div>
        </div>,
        document.body
