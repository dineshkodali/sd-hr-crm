...
export default function AdminAddMember() {
 ...
 {error && <div className="mb-4 text-red-600 bg-red-50 p-2 rounded-xl">{error}</div>}
 {success && <div className="mb-4 text-green-700 bg-green-50 p-2 rounded-xl">{success}</div>}
 ...
 required
 className="mt-1 block w-full rounded-xl border p-2 pr-10"
 placeholder="Min. 8 characters, mix upper/lower, number & special"
 />
 <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 rounded-xl">
 ...
 value={form.role}
 onChange={handleChange}
 className="mt-1 block w-full p-2 rounded-xl border"
 >
