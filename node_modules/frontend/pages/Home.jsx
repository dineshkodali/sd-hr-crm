 ...
 *, *::before, *::after { box-sizing: border-box; }
 ...
 --teal-lt: var(--accent-shadow);
 --teal-xs: var(--accent-shadow);
 ...
 0% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(136, 192, 208, 0.5); }
 70% { transform: scale(1); box-shadow: 0 0 0 10px rgba(136, 192, 208, 0); }
 100% { transform: scale(0.95); box-shadow: 0 0 0 0 rgba(136, 192, 208, 0); }
 }
 ...
 0% { border-color: var(--color-error); box-shadow: 0 0 0 0 rgba(239,68,68,0.3); }
 50% { border-color: var(--color-error); box-shadow: 0 0 0 6px rgba(239,68,68,0); }
 100% { border-color: var(--color-error); box-shadow: 0 0 0 0 rgba(239,68,68,0); }
 }
...
const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
        <div style={{
            background: 'var(--bg-surface)', border: '1.5px solid var(--border-color)', borderRadius: 10,
            padding: '10px 14px', fontSize: 12, boxShadow: '0 20px 40px -8px rgba(0,0,0,0.4)',
        }}>
            <p style={{ color: 'var(--text-secondary)', marginBottom: 6, fontWeight: 600, letterSpacing: '0.03em' }}>{label}</p>
            {payload.map((entry, i) => (
...
const StatCard = ({ title, value, sub, color, icon, delay = 0, onClick, accent }) => {
    ...
    return (
        <div
            ...
            style={{
                background: 'var(--bg-surface)', borderRadius: 16, padding: '22px 22px 20px',
                border: `1.5px solid ${hovered ? color + '44' : 'var(--border-color)'}`,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'relative', zIndex: 2 }}>
                <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>{title}</div>
                    <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em', lineHeight: 1, overflow: 'hidden' }}>
                    ...
                    {sub && (
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--border-color)', display: 'inline-block' }} />
                            {sub}
                        </div>
                    )}
                </div>
...
const MetricCard = ({ title, children, icon, iconBg, delay = 0, onClick }) => {
    ...
    return (
        <div
            ...
            style={{
                background: 'var(--bg-surface)', borderRadius: 16, padding: '22px',
                border: `1.5px solid ${hovered ? 'var(--accent-primary)44' : 'var(--border-color)'}`,
...
const EmptyState = ({ message = "No Data Available" }) => (
    <div style={{
        ...
        height: '100%', color: 'var(--text-secondary)', animation: 'fadeIn 0.4s ease both',
    }}>
        <div style={{
            padding: 14, background: 'var(--bg-primary)', borderRadius: '50%', marginBottom: 12,
...
const MealAnalysisPanel = ({ api, delay = 0 }) => {
    ...
    return (
        <div style={{
            background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{
                        ...
                        border: '1px solid var(--accent-primary)', fontSize: 18, lineHeight: 1,
                    ...
                    <div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>Meal Analysis</div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 1 }}>Service & satisfaction</div>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                    <div style={{ padding: '4px 12px', borderRadius: 99, fontSize: 11, fontWeight: 700, background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                        {total} served
                    </div>
                    <div style={{ padding: '4px 12px', borderRadius: 99, fontSize: 11, fontWeight: 700, background: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                        {avgSat}% satisfaction
                    </div>
                </div>
            </div>
                    ...
                    {mealData.map((m, idx) => {
                        ...
                        return (
                            <div
                                ...
                                onMouseEnter={e => { e.currentTarget.style.background = 'var(--accent-shadow)'; e.currentTarget.style.borderColor = 'var(--accent-primary)33'; }}
                            ...
                            >
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        <div>
                                            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{mealKey}</div>
                                            <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 1 }}>{served} / {ordered} served</div>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <div style={{ flex: 1, background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 99, height: 6, overflow: 'hidden' }}>
                                        <div style={{
                                    ...
                                    <span className="dash-mono" style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', minWidth: 32 }}>{pct}%</span>
                                </div>
                            </div>
                        );
                    })}
...
export default function AdminDashboard() {
    ...
    const PageAnalyticsItem = ({ page, index }) => {
        ...
        return (
            <div
                ...
                style={{
                    background: 'var(--bg-surface)', border: `1.5px solid ${hovered ? 'var(--accent-primary)44' : 'var(--border-color)'}`,
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{page.title}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', background: 'var(--bg-primary)', padding: '2px 8px', borderRadius: 6, border: '1px solid var(--border-color)', fontFamily: "'DM Mono', monospace" }}>
                ...
                <div style={{ width: '100%', height: 44 }}>
                    {!data ? (
                        ...
                        <div style={{ height: '100%', background: 'var(--bg-primary)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--text-secondary)' }}>
                            No data
                        </div>
                    )}
                </div>
    ...
    return (
        <div className="dash-font" style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
            ...
            <div style={{ padding: '32px', maxWidth: 1600, margin: '0 auto' }}>
                ...
                <div style={{
                    ...
                    <div>
                        <h1 className="shimmer-text" style={{ fontSize: 26, fontWeight: 800, letterSpacing: '-0.03em', margin: 0, lineHeight: 1.2, color: 'var(--text-primary)' }}>
                            Analytics Dashboard
                        </h1>
                        <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 5, fontWeight: 500 }}>
                            Overview of your accommodation portfolio
                            {refreshing && <span style={{ marginLeft: 10, fontSize: 11, color: 'var(--accent-primary)', fontWeight: 600 }}>↻ Refreshing…</span>}
                        </p>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {['7D', '30D', '90D', '1Y'].map((range, i) => {
                            ...
                            return (
                                <button key={range} onClick={() => setTimeRange(range.toLowerCase())} style={{
                                    ...
                                    border: active ? 'none' : '1.5px solid var(--border-color)', cursor: 'pointer',
                                    transition: 'all 0.25s cubic-bezier(0.22,1,0.36,1)',
                                    ...(active ? { background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-hover))', color: 'var(--text-on-accent)', boxShadow: '0 4px 15px -3px var(--accent-shadow)' } : { background: 'var(--bg-surface)', color: 'var(--text-secondary)' }),
                                    animation: `slideUp 0.5s cubic-bezier(0.22,1,0.36,1) ${i * 60}ms both`,
                                }} className="rounded-xl">{range}</button>
                            );
                        })}
                        <button style={{ padding: 9, background: 'var(--bg-surface)', border: '1.5px solid var(--border-color)', borderRadius: 10, cursor: 'pointer', color: 'var(--text-secondary)', transition: 'all 0.2s ease' }}
                        ...
                        <select value={selectedProperty} onChange={e => setSelectedProperty(e.target.value)} style={{
                            padding: '9px 16px', fontSize: 13, fontWeight: 600, background: 'var(--bg-surface)',
                            border: '1.5px solid var(--border-color)', borderRadius: 10, color: 'var(--text-primary)', cursor: 'pointer', minWidth: 160, outline: 'none', transition: 'border-color 0.2s ease',
                        }} onFocus={e => { e.target.style.borderColor = 'var(--accent-primary)'; }} onBlur={e => { e.target.style.borderColor = 'var(--border-color)'; }} className="rounded-xl">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 20 }}>
                    <MetricCard title="Total Properties" icon={<I.Building />} iconBg="var(--accent-shadow)" delay={0} onClick={() => navigate('/hotels')}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 8 }}>Registered properties</div>
                    </MetricCard>

                    <MetricCard title="Occupancy Rate" icon={<I.Home />} iconBg="rgba(16, 185, 129, 0.1)" delay={80} onClick={() => navigate('/rooms')}>
                        <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em', lineHeight: 1 }}>
                            <span style={{ animation: 'numberFlip 0.5s cubic-bezier(0.22,1,0.36,1) 380ms both', display: 'inline-block' }}>{occupancyRate}%</span>
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 8, marginBottom: 10 }}>{occupancyLabel}</div>
                        <AnimatedBar percentage={occupancyRate} color="var(--color-success)" delay={200} />
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 8, fontSize: 11, color: 'var(--color-success)', fontWeight: 600 }}>
                            <I.TrendUp /> On target
                        </div>
                    </MetricCard>

                    <MetricCard title="Active Residents" icon={<I.Users />} iconBg="rgba(168, 85, 247, 0.1)" delay={160} onClick={() => navigate('/su/users')}>
                        <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em', lineHeight: 1 }}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 8 }}>Current service users</div>
                    </MetricCard>

                    <MetricCard title="Open Incidents" icon={<I.Alert />} iconBg="rgba(245, 158, 11, 0.1)" delay={240} onClick={() => navigate('/admin/incidents')}>
                        <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em', lineHeight: 1 }}>
                            <span style={{ animation: 'numberFlip 0.5s cubic-bezier(0.22,1,0.36,1) 540ms both', display: 'inline-block' }}>{openIncidentsCount}</span>
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 8 }}>{openIncidentsCount > 0 ? 'Need attention' : 'All clear'}</div>
                        {openIncidentsCount > 0 && (
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 8, fontSize: 11, fontWeight: 700, background: 'rgba(239, 68, 68, 0.1)', color: 'var(--color-error)', padding: '2px 8px', borderRadius: 99, border: '1px solid var(--color-error)', animation: 'pulse-ring 2s ease-in-out infinite' }}>
                                <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--color-error)', display: 'inline-block' }} /> Urgent
                            </div>
                        )}
                    </MetricCard>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20, marginBottom: 20 }}>
                    <MetricCard title="Compliance Health" icon={<I.Shield />} iconBg="rgba(239, 68, 68, 0.1)" delay={80} onClick={() => navigate('/compliance')}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 6 }}>Valid certificates</div>
                        {expiredComplianceCount > 0 && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 8, fontSize: 11, color: 'var(--color-error)', fontWeight: 600 }}>
                    ...
                    <MetricCard title="Expiring Soon" icon={<I.Clock />} iconBg="rgba(245, 158, 11, 0.1)" delay={160} onClick={() => navigate('/compliance')}>
                        <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em' }}>{expiringSoonCount}</div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 6 }}>Certificates within 30 days</div>
                    </MetricCard>

                    <MetricCard title="Pending Maintenance" icon={<I.Wrench />} iconBg="rgba(59, 130, 246, 0.1)" delay={240} onClick={() => navigate('/admin/maintenance')}>
                        <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em' }}>{maintenanceStats.pending || 0}</div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 6 }}>Active work orders</div>
                    </MetricCard>

                    <MetricCard title="Completion Rate" icon={<I.Chart />} iconBg="rgba(16, 185, 129, 0.1)" delay={320}>
                        <div style={{ fontSize: 34, fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.03em' }}>{completionRate}%</div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 6, marginBottom: 10 }}>This month's maintenance</div>
                        <AnimatedBar percentage={completionRate} color="var(--color-success)" delay={200} />
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 8, fontSize: 11, color: 'var(--color-success)', fontWeight: 600 }}>
                ...
                {/* ── ATTENTION ITEMS ── */}
                <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 24, marginBottom: 20, animation: 'slideUp 0.7s cubic-bezier(0.22,1,0.36,1) 400ms both' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                        {displayedAttention.map((item, idx) => (
                            <div key={idx} className="attention-item card-press" onClick={() => {
                                ...
                                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                                padding: '14px 16px', background: 'var(--bg-primary)', borderRadius: 12, cursor: 'pointer',
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <div style={{ padding: 8, background: 'var(--bg-surface)', borderRadius: 10, border: '1px solid var(--color-error)', animation: 'pulse-ring 3s ease-in-out infinite', animationDelay: `${idx * 500}ms` }}>
                                        <I.Shield />
                                    </div>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{item.title || item.name || 'Alert'}</div>
                                        <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>{item.location || item.property_name || 'System'}</div>
                                    </div>
                                </div>
                                <I.ChevronRight />
                            </div>
                ...
                {/* ── MODULE ANALYTICS ── */}
                <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 24, marginBottom: 20, animation: 'slideUp 0.7s cubic-bezier(0.22,1,0.36,1) 440ms both' }}>
                    <SectionHeader icon={<I.Grid />} title="Module Analytics" sub="Operation Hub, HSE, Safeguarding & Escalations" delay={440} />

                    <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 20 }}>
                        <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 20 }} className="hover-lift">
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)' }}>Module Activity</div>
                                <div className="dash-mono" style={{ fontSize: 12, fontWeight: 800, color: 'var(--text-primary)' }}>{total}</div>
                            </div>
                                        ...
                                        <AreaChart data={trendData} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                                            <defs>
                                                <linearGradient id="gradModules" x1="0" y1="0" x2="0" y2="1">
                                                    <stop offset="5%" stopColor="var(--accent-primary)" stopOpacity={0.14} />
                                                    <stop offset="95%" stopColor="var(--accent-primary)" stopOpacity={0} />
                                                </linearGradient>
                                            </defs>
                                            <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" vertical={false} />
                                            <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} interval={6} />
                                            <YAxis axisLine={false} tickLine={false} tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} width={30} />
                                            <Tooltip content={<CustomTooltip />} />
                                            <Area type="monotone" dataKey="activity" name="Activity" stroke="var(--accent-primary)" strokeWidth={2.5} fillOpacity={1} fill="url(#gradModules)" animationDuration={1600} dot={false} activeDot={{ r: 5, fill: 'var(--accent-primary)', strokeWidth: 2, stroke: 'var(--bg-surface)' }} />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                )}
                            </div>
                        ...
                        <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 20 }} className="hover-lift">
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)' }}>Distribution</div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 14, alignItems: 'center' }}>
                                <div style={{ height: 220, position: 'relative' }}>
                                    {moduleLoading ? (
                                        ...
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie data={donutData} dataKey="value" nameKey="name" innerRadius={62} outerRadius={88} paddingAngle={2} stroke="var(--bg-surface)" strokeWidth={2} animationDuration={1400}>
                                    ...
                                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                                        <div style={{ textAlign: 'center' }}>
                                            <div className="dash-mono" style={{ fontSize: 22, fontWeight: 900, color: 'var(--text-primary)' }}>{total}</div>
                                            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Total</div>
                                        </div>
                                    </div>
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                    {donutData.map((d, idx) => (
                                        <div key={d.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 12, background: 'var(--bg-primary)', border: '1px solid var(--border-color)', animation: `slideUp 0.55s cubic-bezier(0.22,1,0.36,1) ${600 + idx * 80}ms both` }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                            ...
                                            <div className="dash-mono" style={{ fontSize: 13, fontWeight: 900, color: 'var(--text-primary)' }}>{Number(d.value) || 0}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                        {modulePageGroups.map((group, gidx) => {
                            ...
                            return (
                                <div key={group.title} style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 18 }} className="hover-lift">
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                                        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)' }}>{group.title} Distribution</div>
                                        <div style={{ fontSize: 11, color: 'var(--text-secondary)', fontWeight: 600 }}>Records</div>
                                    </div>
                                    <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 12, alignItems: 'center' }}>
                                        <div style={{ height: 200, position: 'relative' }}>
                                            {moduleLoading ? (
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <PieChart>
                                                        <Pie data={items} dataKey="value" nameKey="name" innerRadius={56} outerRadius={80} paddingAngle={2} stroke="var(--bg-surface)" strokeWidth={2} animationDuration={1200}>
                                            ...
                                            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                                                <div style={{ textAlign: 'center' }}>
                                                    <div className="dash-mono" style={{ fontSize: 20, fontWeight: 900, color: 'var(--text-primary)' }}>{totalVal}</div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                            {(items || []).map((d, idx) => (
                                                <div key={d.key} style={{ animation: `slideUp 0.6s cubic - bezier(0.22, 1, 0.36, 1) ${700 + gidx * 120 + idx * 80}ms both` }}>
                                                    <div className="card-press" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 12, background: 'var(--bg-surface)', border: '1px solid var(--border-color)', cursor: 'pointer' }}>
                                                        <div onClick={() => navigate(d.route)} style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
                                                            ...
                                                            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.name}</div>
                                                        </div>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                            <div className="dash-mono" style={{ fontSize: 13, fontWeight: 900, color: 'var(--text-primary)' }}>{Number(d.value) || 0}</div>
                                                            ...
                                                            }} className="card-press" style={{ width: 28, height: 28, borderRadius: 10, background: 'var(--bg-surface)', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)' }} aria-label="Toggle page analysis">
                                                    ...
                                                    {expandedAnalyticsPages === d.key && (
                                                        <div style={{ marginTop: 10, padding: 12, borderRadius: 12, background: 'var(--bg-surface)', border: '1px solid var(--border-color)' }}>
                                                                ...
                                                                return (
                                                                    <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 12, alignItems: 'center' }}>
                                                                        <div style={{ height: 160, position: 'relative' }}>
                                                                            <ResponsiveContainer width="100%" height="100%">
                                                                                <PieChart>
                                                                                    <Pie data={donutRows} dataKey="value" nameKey="name" innerRadius={48} outerRadius={70} paddingAngle={2} stroke="var(--bg-surface)" strokeWidth={2} animationDuration={900}>
                                                                            ...
                                                                            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                                                                                <div style={{ textAlign: 'center' }}>
                                                                                    <div className="dash-mono" style={{ fontSize: 18, fontWeight: 900, color: 'var(--text-primary)' }}>{totalStatus}</div>
                                                                                    <div style={{ fontSize: 10, fontWeight: 700, color: '#94a3b8', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Total</div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                                                            {(statusItems.length ? statusItems : [{ key: 'none_row', name: 'No status data', value: 0, color: '#e2e8f0' }]).map((s, si) => (
                                                                                <div key={s.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 12, background: 'var(--bg-primary)', border: '1px solid var(--border-color)', animation: `slideUp 0.5s cubic - bezier(0.22, 1, 0.36, 1) ${80 + si * 60}ms both` }}>
                                                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                                                    ...
                                                                                    <div className="dash-mono" style={{ fontSize: 13, fontWeight: 900, color: 'var(--text-primary)' }}>{Number(s.value) || 0}</div>
                                                                                </div>
                                                                            ))}
                                                                        </div>
                                                                    </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
                    ...
                    <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 24, animation: 'slideUp 0.7s cubic-bezier(0.22,1,0.36,1) 480ms both' }}>
                                ...
                                return (
                                    <div key={property.id} style={{ animation: `slideIn 0.6s cubic - bezier(0.22, 1, 0.36, 1) ${600 + pi * 60}ms both` }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                            <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{property.name}</span>
                                            <span className="dash-mono" style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)' }}>{serviceUsersCount !== null && serviceUsersCount !== undefined ? Number(serviceUsersCount) : `${percentage}% `}</span>
                                        </div>
                    ...
                    <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 24, animation: 'slideUp 0.7s cubic-bezier(0.22,1,0.36,1) 520ms both', display: 'flex', flexDirection: 'column', minHeight: 480 }}>
                        <SectionHeader icon={<I.Wrench />} title="Maintenance Overview" sub="Status distribution" delay={520} />
                        <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
                            {[['var(--text-primary)', 'Pending'], ['#5dcab9', 'In Progress'], ['#a7cdcc', 'Completed']].map(([c, l]) => (
                                <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text-secondary)' }}>
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={maintenanceTrends} margin={{ top: 8, right: 8, left: -16, bottom: 48 }} barCategoryGap="28%">
                                        ...
                                        <XAxis dataKey="label" axisLine={false} tickLine={false} interval={0} height={52} tick={({ x, y, payload }) => {
                                            ...
                                            return <g transform={`translate(${x}, ${y + 4})`}><text textAnchor="middle" fill="var(--text-secondary)" fontSize={10} fontFamily="DM Sans, sans-serif" fontWeight={500}>{short}</text></g>;
                                        }} />
                                        <YAxis axisLine={false} tickLine={false} tickMargin={8} allowDecimals={false} tick={{ fill: 'var(--text-secondary)', fontSize: 11 }} width={32} />
                                        <Tooltip content={<CustomTooltip />} cursor={{ fill: 'var(--accent-shadow)', radius: 4 }} wrapperStyle={{ outline: 'none' }} />
                                        <Bar dataKey="pending" name="Pending" stackId="a" fill="var(--text-primary)" radius={[0, 0, 0, 0]} animationDuration={1400} animationBegin={100} />
                    ...
                    {/* Compliance Donut */}
                    <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 24, animation: 'slideUp 0.7s cubic-bezier(0.22,1,0.36,1) 560ms both' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'stretch', gap: 32 }} onMouseLeave={() => setDonutTip(p => ({ ...p, open: false }))}>
                            <div style={{ flexShrink: 0 }}>
                                <svg width="200" height="200" viewBox="0 0 200 200">
                                    {totalCompliance === 0 ? (
                                        <circle cx="100" cy="100" r="78" fill="none" stroke="var(--bg-primary)" strokeWidth="18" />
                                    ) : (
                                        <>
                                            <circle cx="100" cy="100" r="78" fill="none" stroke="var(--bg-primary)" strokeWidth="18" />
                                    ...
                                    <text x="100" y="96" textAnchor="middle" fontSize="22" fontWeight="800" fill="var(--text-primary)" fontFamily="DM Sans">{totalCompliance}</text>
                                    <text x="100" y="112" textAnchor="middle" fontSize="10" fill="var(--text-secondary)" fontFamily="DM Sans">Total</text>
                                </svg>
                            </div>
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 16, minHeight: 220 }}>
                                {[
                                    { label: 'Valid', value: validComplianceCount, color: '#5dcab9', bg: 'rgba(93, 202, 185, 0.1)' },
                                    { label: 'Expiring Soon', value: expiringSoonCount, color: '#a7cdcc', bg: 'rgba(167, 205, 204, 0.1)' },
                                    { label: 'Expired', value: expiredComplianceCount, color: '#fca5a5', bg: 'rgba(252, 165, 165, 0.1)' },
                                ].map(({ label, value, color, bg }) => (
                                    <div key={label} onClick={() => navigate('/compliance')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 18px', borderRadius: 14, background: 'var(--bg-primary)', cursor: 'pointer', border: '1px solid transparent', transition: 'all 0.2s ease' }}
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <span className="dash-mono" style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)' }}>{Number(value) || 0}</span>
                                            <I.ChevronRight />
                                        </div>
                                    </div>
                                ))}
                                <div style={{ borderTop: '1px solid var(--border-color)', margin: '8px 0' }} />
                                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 14px', fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500 }}>
                                    <span>Total Certificates</span>
                                    <span className="dash-mono" style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{totalCompliance}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {/* ── OVERALL TRENDS ── */}
                <div style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1.5px solid var(--border-color)', padding: 24, marginBottom: 20, animation: 'slideUp 0.7s cubic-bezier(0.22,1,0.36,1) 640ms both' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                        <SectionHeader icon={<I.TrendUp />} title="Overall Trends" sub="Incidents & Resolutions" delay={640} />
                        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                            {[['#3b82f6', 'Incidents'], ['var(--accent-primary)', 'Resolved']].map(([c, l]) => (
                            ...
                            <select style={{ padding: '7px 12px', fontSize: 12, fontWeight: 600, background: 'var(--bg-surface)', border: '1.5px solid var(--border-color)', borderRadius: 8, color: 'var(--text-primary)', cursor: 'pointer', outline: 'none' }} className="rounded-xl">
                        ...
                        {trends.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={trends} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                    ...
                                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" vertical={false} />
                                    <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: 'var(--text-secondary)', fontSize: 12, fontFamily: 'DM Sans' }} />
            ...
            {donutTip.open && (
                <div style={{
                    ...
                    pointerEvents: 'none', background: 'var(--bg-surface)', color: 'var(--text-primary)', borderRadius: 10,
                    padding: '10px 14px', fontSize: 12, boxShadow: '0 20px 40px -8px rgba(0,0,0,0.4)',
                    border: '1px solid var(--border-color)', minWidth: 140, animation: 'scaleIn 0.15s ease both',
                }}>
                    ...
                    <div style={{ color: 'var(--text-secondary)' }}>Count: <span style={{ color: 'var(--text-primary)', fontWeight: 700 }}>{Number(donutTip.value) || 0}</span></div>
                </div>
            )}
        </div>
    );
