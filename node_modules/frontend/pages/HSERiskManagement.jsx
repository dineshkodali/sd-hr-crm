...
if (typeof document !== 'undefined' && !document.getElementById(DELETE_STYLE_ID)) {
    ...
    style.textContent = `
      ...
      tr.hse-risk-deleting {
        animation: hseRiskSlideOut 0.45s cubic-bezier(0.4,0,1,1) forwards;
        overflow: hidden;
        pointer-events: none;
      }
      .hse-risk-card-deleting {
        animation: hseRiskCardDelete 0.45s cubic-bezier(0.4,0,1,1) forwards;
        pointer-events: none;
      }
    `;
...
export default function HSERiskManagement({ user }) {
    ...
    const CATEGORY_STORAGE_KEY = 'hseRiskManagement.customCategories';
    ...
    const closeModal = () => {
        ...
        _openGallery(items, "HSE Risk Documents", "/api/hse/risk-management/attachments");
    };
        ...
        showConfirm(
            ...
            async () => {
                try {
                    ...
                    await api.delete(`/api/hse/risk-management/${id}`).catch(() => null);
                    await refresh();
                } catch (err) {
                    setDeletingIds(prev => { const next = new Set(prev); next.delete(id); return next; });
                    showAlert('Error', 'Delete failed: ' + (err.response?.data?.message || err.message), 'error');
                }
            },
            'danger'
        );
    ...
    const normalizeRiskExportRow = (record) => {
        const base = {
    ...
    const runExport = () => {
        try {
            ...
            const data = (filtered || []).map(normalizeRiskExportRow);
            ...
            console.error('Error exporting HSE risk management:', e);
        ...
        <div className="min-h-screen bg-[var(--bg-primary)] font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
            <div className="p-3 sm:p-4 md:p-6">
                ...
                <div className="mb-6 flex items-start justify-between">
                    <div>
                        <Breadcrumbs items={[{ label: 'Property', path: '/admin/hotels' }, { label: 'HSE Risk Management', path: '/admin/hse-risk-management' }]} />
                        <h1 className="text-3xl font-black text-slate-900 mt-1">HSE Risk Management Dashboard</h1>
                    </div>
                    {hasCreate && (
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="bg-white">
                                    {loading ? (
                                        ...
                                        return (
                                            <tr key={idx} className={`transition-colors ${isDeleting ? 'hse-risk-deleting' : 'hover:bg-[var(--bg-primary)]/60'}`}>
                                                {visibleColumns.checkbox && (
                                                ...
                                                {visibleColumns.reference && (
                                                    <td className="py-5 px-6">
                                                        <span className="text-slate-900 font-semibold text-sm whitespace-nowrap">{r.reference || `RISK-${r.id || idx}`}</span>
                                                    </td>
                                                )}
                                                {visibleColumns.description && (
                                                    <td className="py-5 px-6">
                                                        <div>
                                                            ...
                                                            <div className="text-gray-500 text-xs mt-1 truncate max-w-[200px]">
                                                                {r.title || r.description || "Risk Assessment"}
                                                            </div>
                                                        </div>
                                                    </td>
                                                )}
                                                ...
                                                <div className="p-3 space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
                                                    {statusItems.length === 0 ? (
                                                        <div className="text-center py-8 px-4">
                                                            ...
                                                            <p className="text-[var(--text-secondary)] text-sm">No risks</p>
                                                        </div>
                                                    ) : (
                                                        statusItems.map((risk) => {
                                                            const priorityColor = getPriorityColor(risk.priority || "Medium");
                                                            const isDeleting = deletingIds.has(risk.id);

                                                            return (
                                                                <div
                                                                    key={risk.id}
                                                                    className={`bg-[var(--bg-surface)] rounded-xl p-4 shadow-sm border border-[var(--border-color)] transition-all cursor-pointer ${isDeleting ? 'hse-risk-card-deleting' : ''}`}
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="flex items-center gap-1">
                                                                        <button
                                                                            ...
                                                                            className="flex-1 py-1.5 px-2 bg-gray-50 text-gray-700 rounded-xl text-xs font-medium flex items-center justify-center gap-1"
                                                                            title="View"
                                                                        >
                                                                        ...
                                                                        {hasUpdate && (
                                                                            <button
                                                                                ...
                                                                                className="p-1.5 bg-gray-50 text-gray-700 rounded-xl"
                                                                                title="Edit"
                                                                            >
                ...
                <div className="modal-overlay">
                    <div className="modal-container h-[70vh]">
                        ...
                        <div className="modal-header">
                            <div>
                                <h2 className="modal-title">
                                    {mode === 'create' ? "New Risk Assessment" : mode === 'edit' ? "Edit Assessment" : "Assessment Details"}
                                </h2>
                                <p className="modal-subtitle">
                                    {mode === 'view' ? 'View assessment information' : 'Enter assessment details'}
                                </p>
                            </div>
                        ...
                        {mode === 'view' ? (
                            ...
                            <>
                                <form id="hse-risk-form" onSubmit={submit} className="modal-content form-section">
