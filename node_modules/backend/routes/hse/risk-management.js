...
async function ensureHseRiskAttachments() {
    ...
    try {
        await pool.query("ALTER TABLE public.hse_risk_management ADD COLUMN IF NOT EXISTS attachments JSONB DEFAULT '[]'::jsonb");
    } catch {
    }
    try {
        await pool.query(`
      CREATE TABLE IF NOT EXISTS public.hse_risk_management_attachments (
    ...
    `);
        await pool.query(`CREATE INDEX IF NOT EXISTS idx_hse_risk_management_attachments_risk_id ON public.hse_risk_management_attachments(risk_id)`);
    } catch {
    }
    hseRiskAttachmentsReady = true;
    return true;
}
...
router.get('/', protect, async (req, res) => {
    try {
        await ensureHseRiskAttachments();
...
router.get('/attachments/:id', protect, async (req, res) => {
    try {
       ...
       FROM public.hse_risk_management_attachments
       WHERE id = $1
       LIMIT 1`,
            [Number(id)]
        );
        ...
        console.error('GET /api/hse/risk-management/attachments/:id error:', err);
...
router.delete('/attachments/:id', protect, async (req, res) => {
    try {
       ...
       LIMIT 1`,
        ...
        const riskId = find.rows[0].risk_id;

        await pool.query(`DELETE FROM public.hse_risk_management_attachments WHERE id = $1`, [Number(id)]);
        ...
        if (riskId) {
         ...
         WHERE id = $1`,
                [Number(riskId), Number(id)]
            );
        }

        return res.json({ success: true });
    } catch (err) {
        console.error('DELETE /api/hse/risk-management/attachments/:id error:', err);
...
router.patch('/:id', protect, upload.array('photos', 10), async (req, res) => {
    try {
        ...
        const files = req.files || [];
        const newIds = await insertHseRiskAttachments(updated?.id, files);
        if (newIds.length) {
            await pool.query(
                `UPDATE public.hse_risk_management
         SET attachments = COALESCE(attachments, '[]'::jsonb) || to_jsonb($2::int[])
