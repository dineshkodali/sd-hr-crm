...
// ============================================
// USER <-> ROLES helpers (for employee screens)
// ============================================

// GET roles assigned to a user
router.get("/users/:userId/roles", protect, async (req, res) => {
    if (req.user?.role !== "admin") return res.status(403).json({ message: "Forbidden" });
...
// SET a user's permission role (replace existing). Body: { role_id: number | null }
router.put("/users/:userId/roles", protect, async (req, res) => {
    if (req.user?.role !== "admin") return res.status(403).json({ message: "Forbidden" });
    ...
    try {
        ...
        if (role_id) {
            ...
            // Copy role permissions into user_permissions for backward compatibility with existing behavior
            const permsQuery = await client.query(
                "SELECT module, can_read, can_create, can_update, can_delete FROM role_permissions WHERE role_id = $1",
                [role_id]
            );
           ...
           DO UPDATE SET 
             ...
             can_update = GREATEST(user_permissions.can_update, EXCLUDED.can_update),
             can_delete = GREATEST(user_permissions.can_delete, EXCLUDED.can_delete)`,
                    [userId, perm.module, perm.can_read, perm.can_create, perm.can_update, perm.can_delete]
                );
            }
        }
