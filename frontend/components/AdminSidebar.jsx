...
import { THEMES, applyTheme, applyCustomTheme } from "../src/utils/themeUtils";
...
const Icons = {
  ...
  Palette: () => <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="13.5" cy="6.5" r=".5" fill="currentColor" /><circle cx="17.5" cy="10.5" r=".5" fill="currentColor" /><circle cx="8.5" cy="7.5" r=".5" fill="currentColor" /><circle cx="6.5" cy="12.5" r=".5" fill="currentColor" /><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.9 0 1.7-.1 2.5-.3.8-.2 1.5-.7 1.7-1.5.3-1 .1-2.1-.5-2.9-.6-.8-.8-1.8-.6-2.8.2-1 1.1-1.8 2.1-1.9 2-.2 3.8 1.1 5.2 3.8.2-.8.4-1.6.5-2.4.3-5.5-3.5-10-8.9-10z" /></svg>,
...
export default function AdminSidebar(props) {
  ...
  const menuStructure = [
    ...
    }] : []),
    {
      ...
      items: [
        ...
        { path: "#themes", label: "Themes", icon: <Icons.Palette />, isThemePicker: true },
      ]
    },
  ];
  ...
  const getSubLinkClass = (path) => {
    ...
    return isActive
      ? "text-[var(--accent-primary)] font-bold bg-[var(--accent-shadow)]"
      : "text-[var(--text-secondary)] font-medium hover:text-[var(--text-primary)] hover:bg-[var(--accent-shadow)] hover:font-bold";
  };
  ...
  return (
    <div className="flex h-full relative z-40 admin-sidebar border-r border-[var(--border-color)]" style={{ backgroundColor: 'var(--bg-surface)' }}>

      {/* ICON RAIL */}
      <div className="w-[80px] border-r border-[var(--border-color)] flex flex-col items-center py-3 z-50 shrink-0 h-full" style={{ backgroundColor: 'var(--bg-surface)' }}>
          ...
          {menuStructure.filter(cat => {
            ...
            return (
              <button
                ...
                title={cat.label}
                className="relative group focus:outline-none rounded-xl"
              >
                {(isOpen || isContextActive) && (
                  <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-[var(--accent-primary)] rounded-full border-2 border-[var(--bg-surface)] translate-x-1 -translate-y-1 z-10"></span>
                )}

                <div
                  className={`
                    w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300
                    ${isOpen ? "bg-[var(--accent-shadow)] text-[var(--accent-primary)] shadow-sm ring-1 ring-[var(--accent-primary)]/20" : isContextActive ? "text-[var(--accent-primary)] bg-[var(--bg-primary)]" : "text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-primary)]"}
                  `}
                >
                  {cat.icon}
                </div>
                ...
                {
                  !activeCategory && (
                    <div className="absolute left-full ml-3 px-2 py-1 bg-gray-800 text-white text-xs rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 transition-opacity">
                      {cat.label}
                    </div>
                  )
                }
      ...
      {/* SLIDE-OUT PANEL */}
      <div className={`border-r border-[var(--border-color)] flex flex-col overflow-hidden transition-all duration-300 ease-in-out ${activeCategory ? "w-64 opacity-100" : "w-0 opacity-0"}`} style={{ backgroundColor: 'var(--bg-surface)' }}>
        <div className="w-64 flex flex-col h-full">
          <div className="h-[70px] flex items-center px-6 border-b border-[var(--border-color)] shrink-0">
            ...
            {(() => {
              ...
              return filteredItems.map((item, idx) => {
                if (item.isThemePicker) {
                  return (
                    <div key={idx} className="px-3 py-4 bg-[var(--bg-primary)] rounded-3xl border border-[var(--border-color)] shadow-sm mt-4 space-y-4">
                      <div className="flex flex-wrap items-center justify-center gap-2.5 px-2">
                        {Object.entries(THEMES).map(([key, theme]) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-6 h-6 rounded-full border-2 transition-all shadow-sm ${selectedTheme === key ? 'border-[var(--accent-primary)] scale-110 shadow-md ring-2 ring-[var(--accent-primary)]/20' : 'border-transparent opacity-70 hover:opacity-100 hover:scale-105'}`}
                        ...
                        <div className="relative flex items-center">
                          <input
                            ...
                            className="w-6 h-6 rounded-full border-2 border-gray-200 p-0 bg-transparent cursor-pointer overflow-hidden shadow-sm"
                            title="Custom Color"
                          />
                          {selectedTheme === 'custom' && (
                            <div className="absolute -bottom-1 -right-1 w-2.5 h-2.5 bg-[var(--accent-primary)] rounded-full border-2 border-[var(--bg-surface)] shadow-sm" />
                          )}
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        disabled={selectedTheme === activeTheme && (selectedTheme !== 'custom' || customColor === localStorage.getItem("customThemeColor"))}
                        className={`w-full py-2.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] transition-all
                          ${(selectedTheme === activeTheme && (selectedTheme !== 'custom' || customColor === localStorage.getItem("customThemeColor")))
                            ? "bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-100/50"
                            : "bg-[var(--accent-primary)] text-white hover:bg-[var(--accent-hover)] shadow-lg shadow-[var(--accent-primary)]/20 hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm"}
                        `}
                      >
                        Save Theme
                      </button>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center px-3 py-3 rounded-xl text-sm transition-colors mb-1 whitespace-nowrap ${getSubLinkClass(item.path)}`}
                  >
