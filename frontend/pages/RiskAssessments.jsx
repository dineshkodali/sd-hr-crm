...
export default function RiskAssessments({ user }) {
  ...
  const handleDownloadPDF = () => {
    ...
    const data = filteredAssessments.map(assessment => ({
      ...
      risk_level: assessment.risk_level || 'N/A',
      status: assessment.status || 'N/A',
      assessment_date: assessment.assessment_date ? new Date(assessment.assessment_date).toLocaleDateString() : 'N/A',
      property: assessment.property_name || assessment.hotel_name || 'N/A'
    }));

    generatePDF(data, columns, 'Risk Assessments', 'risk-assessments');
  };
  ...
  const handleDownloadCSV = () => {
    ...
    generateCSV(data, columns, 'risk-assessments');
  };
