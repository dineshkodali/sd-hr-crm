...
export default function ServiceUsersList({ user, openAddModal = false }) {
  ...
  const dynamicColumns = useMemo(() => {
    ...
    return cols
      ...
      .map((c) => ({
        ...c,
        _name: String(c?.column_name || '').trim(),
      }))
      .sort((a, b) => String(a?.ordinal_position || 0) - String(b?.ordinal_position || 0));
  }, [suColumns, RESERVED_FORM_KEYS, HIDDEN_DYNAMIC_KEYS]);

  const exportColumns = useMemo(() => {
    const custom = (dynamicColumns || []).map((c) => {
      const name = c?._name || c?.column_name;
      return {
        header: String(name || '').replace(/_/g, ' ').replace(/\b\w/g, (m) => m.toUpperCase()),
        key: name,
      };
    }).filter((c) => c.key);

    return [...BASE_EXPORT_COLUMNS, ...custom];
  }, [BASE_EXPORT_COLUMNS, dynamicColumns]);
  ...
  const normalizeExportRow = (record) => {
    const safe = record || {};
    const property = safe?.hotel_name || safe?.property_name || safe?.property || safe?.hotel || '';
    const dob = safe?.date_of_birth || safe?.dob || '';
    const base = {
      first_name: safe?.first_name ?? '',
      last_name: safe?.last_name ?? '',
      date_of_birth: dob ? new Date(dob).toLocaleDateString() : '',
      nationality: safe?.nationality ?? '',
      gender: safe?.gender ?? '',
      status: safe?.status ?? '',
      property,
      room_number: safe?.room_number ?? safe?.room ?? '',
    };

    for (const col of dynamicColumns || []) {
      const k = col?._name || col?.column_name;
      if (!k) continue;
      base[k] = safe?.[k] ?? '';
    }
    return base;
  ...
  const runExport = () => {
    try {
      ...
      const data = (filteredUsers || []).map(normalizeExportRow);

      if (exportFormat === 'pdf') {
        generatePDF(data, columns, 'Service Users', 'service-users');
      } else if (exportFormat === 'csv') {
        generateCSV(data, columns, 'service-users');
      }

      closeExport();
    } catch (e) {
      console.error('Error exporting service users:', e);
  ...
  const renderDynamicField = (col) => {
    ...
    if (inputType === 'checkbox') {
      ...
      return (
        <div key={key} className="form-group">
          <label className="block text-sm font-semibold text-slate-700 mb-2">{label}</label>
          <div className="w-full h-[42px] flex items-center !justify-start !px-0 text-left">
            <input
              type="checkbox"
              name={key}
              checked={Boolean(formData?.[key])}
              className="w-5 h-5 !m-0 mr-auto"
              onChange={(e) => {
  ...
  if (permissionsLoading) {
    return (
      <div className="-m-6 p-6 min-h-screen relative font-sans bg-[var(--bg-primary)]">
