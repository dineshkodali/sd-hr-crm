...
if (typeof document !== 'undefined' && !document.getElementById(DELETE_STYLE_ID)) {
    ...
    style.textContent = `
      ...
      @keyframes maintenanceCardDelete {
      ...
      tr.maintenance-deleting {
        animation: maintenanceSlideOut 0.45s cubic-bezier(0.4,0,1,1) forwards;
        overflow: hidden;
        pointer-events: none;
      }
      .maintenance-card-deleting {
        animation: maintenanceCardDelete 0.45s cubic-bezier(0.4,0,1,1) forwards;
        pointer-events: none;
      }
    `;
...
function getAvatarColor(name) {
    ...
    const openAttachmentsGallery = (items = []) => {
        ...
        _openGallery(items, "Maintenance Documents", "/api/maintenance/attachments");
    };
...
const FormSelect = ({ label, value, onChange, options, required = false, disabled = false }) => (
    <div className="w-full">
        <label className="block text-sm font-semibold text-slate-700 mb-2">
            {label} {required && <span className="text-red-500">*</span>}
        </label>
        <select
            ...
            onChange={onChange}
            disabled={disabled}
            className={`w-full border border-gray-300 rounded-xl -xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 bg-white ${disabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
        >
            {options}
        </select>
    </div>
...
export default function MaintenancePage({ user }) {
    ...
    useEffect(() => {
        ...
        const fetchAvailableColumns = async () => {
            try {
                ...
                const columnNames = Array.from(new Set(cols.map(col => {
                ...
                const EXCLUDED_FORM_CUSTOM_COLUMNS = new Set([
                    'id',
                    'title',
                    'room',
                    'category',
                ...
                const customCols = Array.from(new Set(columnNames
                    .filter(col => !standardCols.includes(col))
                    .filter((col) => {
                        const normalized = String(col).trim().toLowerCase();
                        return !EXCLUDED_FORM_CUSTOM_COLUMNS.has(normalized);
                    })));
    ...
    useEffect(() => {
        setCategorySelectValue(form.category || '');
        if (form.category) {
            setShowAddCategory(false);
        }
    }, [form.category]);
    ...
    useEffect(() => {
        const isModalOpen = showCreate || showEdit || showView || confirmDialog.isOpen;
        if (isModalOpen) {
    ...
    }, [showCreate, showEdit, showView, confirmDialog.isOpen]);
    ...
    async function handleDelete(id) {
        ...
        setConfirmDialog({
            ...
            onConfirm: async () => {
                try {
                    const record = (tasks || []).find((t) => String(t.id) === String(id)) ?? null;
                    ...
                    await api.delete(`/api/maintenance/${id}`).catch(() => null);
                    await loadTasks();
                } catch (error) {
    ...
    async function handleCreateSubmit(e) {
        ...
        try {
            const missing = [];
            if (!String(form.title || '').trim()) missing.push('Task Title');
            if (!form.room) missing.push('Room / Area');
            if (!form.hotelId) missing.push('Property');
            ...
            if (!String(form.start || '').trim()) missing.push('Start Date');
            if (!String(form.dueDate || '').trim()) missing.push('Due Date');
            if (!String(form.raisedBy || '').trim()) missing.push('Raised By');
            if (!form.assignedTo) missing.push('Assigned To');
            if (!String(form.action || '').trim()) missing.push('Action Required');
            ...
            customColumns.forEach(col => {
                if (form[col] !== undefined) {
                    ...
                    const inputType = meta.input_type || 'text';
                    if (inputType === 'checkbox') {
                        if (form[col] === true || String(form[col]).toLowerCase() === 'true' || String(form[col]) === 'true') createData[col] = true;
                        else if (form[col] === false || String(form[col]).toLowerCase() === 'false' || String(form[col]) === 'false') createData[col] = false;
                        else createData[col] = null;
                    } else {
        ...
        try {
            ...
            customColumns.forEach(col => {
                if (form[col] !== undefined) {
                    ...
                    const inputType = meta.input_type || 'text';
                    if (inputType === 'checkbox') {
                        if (form[col] === true || String(form[col]).toLowerCase() === 'true' || String(form[col]) === 'true') updateData[col] = true;
                        else if (form[col] === false || String(form[col]).toLowerCase() === 'false' || String(form[col]) === 'false') updateData[col] = false;
                        else updateData[col] = null;
                    } else {
        ...
        <div className="min-h-screen bg-[var(--bg-primary)] font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
            <div className="p-3 sm:p-4 md:p-6">
                ...
                <div className="mb-6 flex items-start justify-between">
                    <div>
                        <Breadcrumbs items={[{ label: 'Property', path: '/admin/hotels' }, { label: 'Maintenance', path: '/admin/maintenance' }]} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div className="bg-white rounded-xl p-5 flex items-center gap-4 border border-gray-100 transition-all duration-200">
                        ...
                        <div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Total Orders</div>
                    ...
                    <div className="bg-white rounded-xl p-5 flex items-center gap-4 border border-gray-100 transition-all duration-200">
                        ...
                        <div>
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">In Progress</div>
                            <div className="text-2xl font-black text-slate-800 leading-none">{stats.inProgress}</div>
                        </div>
                    </div>
                    <div className="bg-white rounded-xl p-5 flex items-center gap-4 border border-gray-100 transition-all duration-200">
                        ...
                        <div className="mb-6">
                            <div className="flex items-center justify-between mb-4">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-400">Manage and track your maintenance tasks</p>
                                </div>
                                <div className="flex items-center gap-3">
                                    {/* Search Input */}
                                    ...
                                    {loading ? (
                                        ...
                                        return (
                                            <tr key={row.id} className={`transition-colors border-b border-gray-100 last:border-0 ${isDeleting ? 'maintenance-deleting' : ''}`}>
                                                ...
                                                <td className="py-4 px-6 text-right sticky right-0 z-10 bg-white" style={{ boxShadow: '-2px 0 5px -2px rgba(0,0,0,0.08)' }}>
                                                    <div className="flex items-center justify-end gap-1 transition-opacity">
                                                        <button
                                                            onClick={() => openView(row)}
                                                            className="p-1.5 text-slate-400 rounded-xl transition-colors border-0"
                                                            title="View"
                                                        >
                                                            <Eye className="w-4 h-4" />
                                                        </button>
                                        ...
                                        <tr>
                                            <td colSpan={100} className="py-12 text-center text-slate-500 font-medium">No work orders found.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                                ...
                                {showView ? (
                                    <div className="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-sm text-gray-700 leading-relaxed min-h-[100px]">
                                    ...
                                    <form id="maintenance-form" onSubmit={showEdit ? handleEditSubmit : handleCreateSubmit}>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <FormSelect
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        if (val === ADD_NEW_CATEGORY_VALUE) {
                                                            setCategorySelectValue(ADD_NEW_CATEGORY_VALUE);
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                setCategoryOptions((prev) => {
                                                                    ...
                                                                    if (prevArr.includes(trimmed)) return prevArr;
                                                                    return [...prevArr, trimmed];
                                                                });
                                                                setCategorySelectValue(trimmed);
                                                                setShowAddCategory(false);
                                                                handleFormChange("category", trimmed);
                                                                setNewCategory('');
                                                            }}
                                            ...
                                            {/* Row 5 */}
                                            <div className="w-full">
                                                <label className="block text-sm font-semibold text-slate-700 mb-2">Raised By <span className="text-red-500">*</span></label>
                                                <input
                                                    type="text"
                                                    value={form.raisedBy}
                                                    readOnly
                ...
                showView && viewingTask && (
                    <div className="modal-overlay">
                        <div className="modal-container h-[70vh]">
                            <div className="modal-header">
                                <div>
                                    <h2 className="modal-title">Work Order Details</h2>
                                    <p className="modal-subtitle">View maintenance work order information</p>
                                </div>
                                <button
                            ...
                            <div className="modal-content text-left">
                                <div className="form-grid-2">
                                    <DetailField
                                        label="Reference"
                                        value={`MNT-2025-${viewingTask.ref || (viewingTask.id ? Number(viewingTask.id).toString(36).padStart(8, '0').slice(-8) : '')}`}
                                    />
                                    <DetailField label="Status" value={viewingTask.status} />
                                    <DetailField label="Title" value={viewingTask.title} />
                                    <DetailField label="Property" value={viewingTask.hotel || viewingTask.hotelName} />
                                    <DetailField label="Room" value={viewingTask.room} />
                                    <DetailField label="Category" value={viewingTask.category} />
                                    <DetailField label="Priority" value={viewingTask.priority} />
                                    <DetailField label="Raised By" value={viewingTask.raisedBy} />
                                    <DetailField label="Start Date" value={formatDate(viewingTask.start)} />
                                    <DetailField label="Due Date" value={formatDate(viewingTask.dueDate)} />
                                    ...
                                    {(customColumns || []).map((col) => {
                                        ...
                                        const rawVal = viewingTask?.raw?.[col] ?? viewingTask?.[col];
