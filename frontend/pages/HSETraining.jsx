...
if (typeof document !== 'undefined' && !document.getElementById(DELETE_STYLE_ID)) {
    ...
    style.textContent = `
      ...
      @keyframes hseTrainingCardDelete {
      ...
      tr.hse-training-deleting {
        animation: hseTrainingSlideOut 0.45s cubic-bezier(0.4,0,1,1) forwards;
        overflow: hidden;
        pointer-events: none;
      }
      .hse-training-card-deleting {
        animation: hseTrainingCardDelete 0.45s cubic-bezier(0.4,0,1,1) forwards;
        pointer-events: none;
      }
    `;
...
const categoryOptions = ['Induction', 'Fire Safety', 'Manual Handling', 'First Aid', 'Equipment', 'Other'];
const priorities = ['Low', 'Medium', 'High', 'Urgent'];

export default function HSETraining({ user }) {
    ...
    // Get permissions for hse_training module
    const { canRead, canCreate, canUpdate, canDelete } = usePermissions(currentUser);
    const hasRead = canRead("hse_training");
    const hasCreate = canCreate("hse_training");
    const hasUpdate = canUpdate("hse_training");
    const hasDelete = canDelete("hse_training");
    ...
    const CATEGORY_STORAGE_KEY = 'hseTraining.customCategories';
    ...
    // Default visible columns for HSE Training (must match other pages)
    ...
    const [visibleColumns, setVisibleColumns] = useState(() => {
        try {
            const saved = localStorage.getItem('hseTrainingVisibleColumns');
            if (saved) {
    ...
    useEffect(() => {
        try {
            localStorage.setItem('hseTrainingVisibleColumns', JSON.stringify(visibleColumns));
        } catch (e) {
    ...
    const fetchAvailableColumns = async () => {
        try {
            const res = await api.get('/api/forms-builder/tables/hse_training/columns');
            ...
            // System and known HSE Training columns to exclude
            const systemColumns = [
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                ...
                const r2 = await api.get('/api/hse/training?limit=500').catch(() => ({ data: [] }));
                if (mounted) setRecords(Array.isArray(r2?.data) ? r2.data : (r2?.data?.rows ?? r2?.data ?? []));
            } catch (err) {
                console.warn('load HSE training failed', err);
    ...
    const refresh = async () => { try { setLoading(true); const r = await api.get('/api/hse/training?limit=500'); setRecords(Array.isArray(r?.data) ? r.data : (r?.data?.rows ?? r?.data ?? [])); } catch (err) { console.warn('refresh failed', err); } finally { setLoading(false); } };
    ...
    const openAttachmentsGallery = (items = []) => {
        ...
        const urls = items.map((x) => {
            ...
            const u = isNumericId ? `/api/hse/training/attachments/${x}` : String(x);
            return /^https?:\/\//i.test(u) ? u : `${base}${u.startsWith('/') ? '' : '/'}${u}`;
        });
        const safeTitle = `Training Photos (${urls.length})`;
        const html = `
      <!doctype html>
      <html lang="en">
        <head>
    ...
    const removeAttachment = async (attachmentId) => {
        ...
        try {
            await api.delete(`/api/hse/training/attachments/${attachmentId}`);
            await refresh();
    ...
    const submit = async (e) => {
        ...
        try {
            ...
            if (mode === 'create') await api.post('/api/hse/training', fd);
            else await api.patch(`/api/hse/training/${selected?.id}`, fd);
            await refresh();
            closeModal();
        } catch (err) {
    ...
    const doDelete = async (id) => {
        ...
        setConfirmDialog({
            ...
            message: 'Delete this training record? This action cannot be undone.',
            type: 'danger',
            onConfirm: async () => {
                try {
                    ...
                    await api.delete(`/api/hse/training/${id}`).catch(() => null);
                    await refresh();
                } catch (err) {
    ...
    const normalizeTrainingExportRow = (record) => {
        const base = {
    ...
    const runExport = () => {
        try {
            ...
            const data = (filtered || []).map(normalizeTrainingExportRow);

            if (exportFormat === 'pdf') {
                generatePDF(data, columns, 'HSE Training Records', 'hse-training');
            } else if (exportFormat === 'csv') {
                generateCSV(data, columns, 'hse-training');
            }

            closeExport();
        } catch (e) {
            console.error('Error exporting HSE training:', e);
            ...
            <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
                <div className="p-3 sm:p-4 md:p-6">
                    ...
                    <div className="mb-6 flex items-start justify-between">
                        <div>
                            <Breadcrumbs items={[{ label: 'Property', path: '/admin/hotels' }, { label: 'HSE Training', path: '/admin/hse-training' }]} />
                            <h1 className="text-3xl font-black text-slate-900 mt-1">HSE Training Dashboard</h1>
                        </div>
                        {hasCreate && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                        ...
                        <div className="bg-white rounded-xl p-5 border border-gray-100 flex items-center gap-4 transition-all duration-200">
                            <div className="bg-rose-50 text-rose-600 h-14 w-14 rounded-xl flex items-center justify-center shrink-0">
                                <GraduationCap className="w-7 h-7" />
                            </div>
                            <div className="flex-1 min-w-0">
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody className="bg-white">
                                        {loading ? (
                                            ...
                                            return (
                                                <tr key={idx} className={`transition-colors ${isDeleting ? 'hse-training-deleting' : ''}`}>
                                                    {visibleColumns.checkbox && (
                                                    ...
                                                    {visibleColumns.reference && (
                                                        <td className="py-5 px-6">
                                                            <span className="text-slate-900 font-semibold text-sm whitespace-nowrap">{r.reference || `TRN-${r.id || idx}`}</span>
                                                        </td>
                                                    )}
                                                    {visibleColumns.description && (
                                                        <td className="py-5 px-6">
                                                                    ...
                                                                    <div className="flex items-center gap-2">
                                                                        <div className="flex items-center -space-x-2">
                                                                            {thumbs.map((id) => (
                                                                                <img
                                                                                    key={String(id)}
                                                                                    src={`${base}/api/hse/training/attachments/${id}`}
                                                                                    alt={`Attachment ${id}`}
                                                                                    className="w-9 h-9 rounded-xl border border-white object-cover shadow-sm"
                                                                                    loading="lazy"
                                            ...
                                            <tr>
                                                <td colSpan="9" className="py-8 text-center text-gray-500">No training tasks found.</td>
                                            </tr>
                                        )}
                                    </tbody>
                                </table>
                            ...
                            <div className="overflow-x-auto scrollbar-hide -mx-6 px-6">
                                <div className="flex gap-4 min-w-max pb-4">
                                    {['Scheduled', 'In Progress', 'Completed'].map((status) => {
                                        const statusItems = filtered.filter((training) => {
                                            return (training.status || 'Scheduled').toLowerCase() === status.toLowerCase();
                                        });
                                                    ...
                                                    <div className="p-3 space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
                                                        {statusItems.length === 0 ? (
                                                            ...
                                                            statusItems.map((training) => {
                                                                const priorityColor = getPriorityColor(training.priority || "Medium");
                    ...
                    <div className="modal-overlay">
                        <div className="modal-container h-[70vh]">
                            ...
                            <div className="modal-header">
                                <div>
                                    <h2 className="modal-title">
                                        {mode === 'create' ? "New HSE Training" : mode === 'edit' ? "Edit Training" : "Training Details"}
                                    </h2>
                                    <p className="modal-subtitle">
                                        {mode === 'view' ? 'View training information' : 'Enter training details'}
                                    </p>
                                </div>
                            ...
                            {mode === 'view' ? (
                                ...
                                <>
                                    <form id="hse-training-form" onSubmit={submit} className="modal-content form-section">
                                        ...
                                        <div className="form-grid-2">
                                            ...
                                            <div className="col-span-1">
                                                ...
                                                <select
                                                    ...
                                                    onChange={(e) => {
                                                        const h = hotels.find(x => String(x.id) === String(e.target.value));
                                                        setFormData({ ...formData, property_id: e.target.value, property_name: h ? h.name : '' });
                                                    }}
