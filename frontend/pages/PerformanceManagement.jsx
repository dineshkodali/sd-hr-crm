...
/* src/pages/PerformanceManagement.jsx */
...
const SAMPLE = [
  { id: 1, title: "Passport & VISA Verification", reference: "PMT-2025-e5198a6e", description: "Operation work required as per inspection report.", priority: "Medium", status: "Completed", assignedTo: "ABC Maintenance", date: "2025-02-08", type: "Performance Management" },
  { id: 2, title: "Resident Data Update", reference: "PMT-2025-c51690eb", description: "Operation work required as per inspection report.", priority: "Medium", status: "Pending", assignedTo: "Unassigned", date: "2025-09-26", type: "Performance Management" },
  { id: 3, title: "AIRE Annual Reporting", reference: "PMT-2025-cda9bd4e", description: "Operation work required as per inspection report.", priority: "Low", status: "Completed", assignedTo: "In-house Team", date: "2025-06-02", type: "Performance Management" },
  { id: 4, title: "Immigrant Status Validation", reference: "PMT-2025-2b9c6ef8", description: "Operation work required as per inspection report.", priority: "Urgent", status: "Completed", assignedTo: "Quick Fix Services", date: "2025-03-17", type: "Performance Management" },
  { id: 5, title: "Address Verification Support", reference: "PMT-2025-f3c0c417", description: "Operation work required as per inspection report.", priority: "Low", status: "Pending", assignedTo: "Unassigned", date: "2024-12-19", type: "Performance Management" },
  { id: 6, title: "Residency Renewal Assistance", reference: "PMT-2025-ca81318f", description: "Operation work required as per inspection report.", priority: "Medium", status: "Completed", assignedTo: "Quick Fix Services", date: "2024-10-24", type: "Performance Management" },
  { id: 7, title: "Fix extractor fan", reference: "PMT-2025-576b7ce4", description: "Maintenance work required as per inspection report.", priority: "Low", status: "Pending", assignedTo: "Unassigned", date: "2025-04-24", type: "Performance Management" },
];
...
export default function PerformanceManagement() {
  ...
  // Get permissions for performance module (matches AccessManagement MODULES keys)
  const { canRead, canCreate, canUpdate, canDelete } = usePermissions(currentUser);
  const hasRead = canRead("performance");
  const hasCreate = canCreate("performance");
  const hasUpdate = canUpdate("performance");
  const hasDelete = canDelete("performance");
  ...
  const fetchHotels = useCallback(async (signal) => {
    try {
      ...
      if (!isCanceled) {
        console.error("Performance fetchHotels error:", err);
        setHotels([]);
      }
    } finally {
  ...
  const loadTasks = useCallback(async (signal) => {
    setLoading(true);
    try {
      const res = await api.get("/api/performance-management", { signal, params: { limit: 200 } });
      ...
      const formattedTasks = mapped.map((t) => ({
        id: t.id,
        title: t.title ?? "",
        reference: t.reference ?? `PMT-2025-${String(t.id).slice(0, 4)}`,
        ...
        type: t.type || "Performance Management",
        raw: t,
      }));
      ...
      console.error("Performance loadTasks error:", err);
      setTasks(SAMPLE);
      setLoading(false);
    }
  }, []);
  ...
  async function handleDelete(id) {
    ...
    try {
      await api.delete(`/api/performance-management/${id}`);
  ...
  async function handleCreateSubmit(e) {
    ...
    setCreating(true);
    try {
      const res = await api.post("/api/performance-management", form);
      const newTask = {
        id: res.data.data.id,
        title: form.title,
  ...
  async function handleEditSubmit(e) {
    ...
    setEditing(true);
    try {
      await api.put(`/api/performance-management/${editingId}`, form);
      const updatedTask = {
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-6">
        <div className="max-w-[1500px] mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500 mt-1">
                <IconHome size={14} /> <span>/</span> <span>Employees</span> <span>/</span> <span className="text-orange-500 font-medium">Overview</span>
              </div>
            </div>
            {hasCreate && (
              <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <KPICard title="Total Tasks" count={stats.total} color="blue" icon={<IconBuilding size={20} />} trend="+2 new" />
          ...
          <div className="p-5 border-b border-slate-100 bg-white">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-800">Task List</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                  <th className="p-4 w-10 text-center"><input type="checkbox" className="rounded border-slate-300 accent-orange-500 w-4 h-4 cursor-pointer" /></th>
                  <th className="p-4 w-32">Reference</th>
                  <th className="p-4 min-w-[250px]">Task Details</th>
              ...
              <tbody className="text-sm divide-y divide-slate-50">
                {loading ? (
                  <tr><td colSpan="8" className="p-12 text-center text-slate-400">Loading tasks...</td></tr>
                  ...
                  <tr>
                    <td colSpan="8" className="p-12 text-center">
                        <div className="flex flex-col items-center justify-center text-slate-400">
                            ...
                            <button onClick={() => setShowCreate(true)} className="mt-4 text-orange-500 hover:text-orange-600 text-sm font-medium">Create New Task</button>
                        </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl relative border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">Fill in the details below to track performance.</p>
              </div>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="e.g. Inspect HVAC System in Block B"
                            value={form.title} 
