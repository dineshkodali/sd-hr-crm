...
export default function AIRETasks({ user }) {
    ...
    function getInitials(name) {
        ...
        _openGallery(items, "AIRE Task Documents", "/api/aire-tasks/attachments");
    };
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-[var(--bg-primary)] border-b border-[var(--border-color)]">
                                    <tr>
                                        ...
                                        {visibleColumns.type && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">TYPE</th>}
                                        {visibleColumns.reference && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">REFERENCE</th>}
                                        {visibleColumns.description && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">DESCRIPTION</th>}
                                        {visibleColumns.attachments && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">ATTACHMENTS</th>}
                                        {visibleColumns.priority && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">PRIORITY</th>}
                                        {visibleColumns.status && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">STATUS</th>}
                                        {visibleColumns.assigned && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">ASSIGNED TO</th>}
                                        {visibleColumns.date && <th className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">DATE</th>}
                                        {customColumns.filter(col => visibleColumns[col]).map(col => (
                                            <th key={col} className="py-4 px-4 text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">
                                                {col.replace(/_/g, ' ').toUpperCase()}
                                            </th>
                                        ))}
                                        <th className="py-4 px-6 text-center text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider sticky right-0 z-10 bg-[var(--bg-primary)]" style={{ boxShadow: '-2px 0 5px -2px rgba(0,0,0,0.08)' }}>ACTIONS</th>
                                    </tr>
                                </thead>
                                                ...
                                                <div className="p-3 space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
                                                    {statusItems.length === 0 ? (
                                                        <div className="text-center py-8 px-4">
                                                            <CheckSquare className="w-10 h-10 mx-auto mb-2 text-[var(--text-secondary)]/40" />
                                                        ...
                                                        statusItems.map((task) => {
                                                            ...
                                                            return (
                                                                <div
                                                                    key={task.id}
                                                                    className={`bg-[var(--bg-surface)] rounded-xl p-4 shadow-sm border border-[var(--border-color)] hover:shadow-md transition-all cursor-pointer ${isDeleting ? 'airetask-card-deleting' : ''}`}
                                                                    onClick={() => { setEditingTask(task); setIsViewing(true); setShowModal(true); }}
                                                                >
                                                                    <div className="flex items-center justify-between mb-2">
                                                                        <span className="text-xs font-mono text-[var(--text-secondary)]/60">{task.reference || `TASK-${task.id}`}</span>
...

                                                                    {
                                                                        task.task_type && (
                                                                            <div className="mb-3">
                                                                                <span className="inline-flex items-center gap-1 px-2 py-1 rounded-xl text-xs font-medium border border-[var(--border-color)]" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--color-info)' }}>
                                                                                    {task.task_type}
                                                                                </span>
                                                                            </div>
                                                                        )
                                                                    ...
                                                                    <div className="flex items-center justify-between pt-3 border-t border-[var(--border-color)] mb-2">
                                                                        <div className="flex items-center gap-2">
                                                                            {task.assigned_to ? (
                                                                                <>
                                                                                    <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--accent-primary)' }}>
                                                                    ...
                                                                    <div className="flex items-center gap-1">
                                                                        <button
                                                                            ...
                                                                            className="flex-1 py-1.5 px-2 bg-[var(--bg-primary)] text-[var(--text-primary)] hover:bg-[var(--bg-primary)]/80 rounded-xl transition-colors text-xs font-medium flex items-center justify-center gap-1"
                                                                            title="View"
                                                                        >
                                                                        ...
                                                                        {hasUpdate && (
                                                                            <button
                                                                                ...
                                                                                className="p-1.5 bg-[var(--bg-primary)] text-[var(--text-primary)] hover:bg-[var(--bg-primary)]/80 rounded-xl transition-colors"
                                                                                title="Edit"
                                                                            >
                                                                        ...
                                                                        {hasDelete && (
                                                                            <button
                                                                                ...
                                                                                className="p-1.5 bg-[var(--bg-primary)] text-[var(--text-primary)] hover:bg-red-50 hover:text-red-600 rounded-xl transition-colors"
                                                                                title="Delete"
                                                                            >
