...
const VCSOrganisations = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await api.put(`/api/vcs-organisations/${editingId}`, formData);
      } else {
        await api.post('/api/vcs-organisations', formData);
      }
      setShowForm(false);
      fetchOrganisations();
    } catch (err) {
  ...
  const handleDeleteClick = async (organisation) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          await api.delete(`/api/vcs-organisations/${organisation.id}`);
  ...
  const handleDownloadPDF = () => {
    ...
    const data = filteredOrganisations.map(org => ({
      reference: org.reference || 'N/A',
      name: org.name || 'N/A',
      priority: org.priority || 'N/A',
      status: org.status || 'N/A',
      date: org.created_at ? new Date(org.created_at).toLocaleDateString() : 'N/A',
      property: org.property_name || org.hotel_name || 'N/A'
    }));

    generatePDF(data, columns, 'VCS Organisations', 'vcs-organisations');
  };
  ...
  const handleDownloadCSV = () => {
    ...
    generateCSV(data, columns, 'vcs-organisations');
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 font-sans" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="p-3 sm:p-4 md:p-6 w-[90%] max-w-[1800px] mx-auto">
        ...
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">VCS Organisations</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-500 text-sm mb-1">Total Organisations</div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-500 text-sm mb-1">Active Partnerships</div>
              <div className="text-2xl font-bold text-gray-900">{statsData.active}</div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-500 text-sm mb-1">High Risk Partners</div>
              <div className="text-2xl font-bold text-gray-900">{statsData.highRisk}</div>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-gray-100 flex items-center gap-4 transition-all duration-200 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-gray-500 text-sm mb-1">Referrals This Month</div>
              <div className="text-2xl font-bold text-gray-900">{statsData.referrals}</div>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto -mx-6 px-6">
              <div className="flex gap-4 min-w-max pb-4">
                {['Active', 'Pending', 'Inactive'].map((status) => {
                  const statusItems = filteredOrganisations.filter((org) => {
                        ...
                        <div className="p-3 space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto">
                          {statusItems.length === 0 ? (
                            <div className="text-center py-8 px-4">
                              ...
                              <p className="text-gray-400 text-sm">No organisations</p>
                            </div>
                          ) : (
                            statusItems.map((org) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs font-mono text-gray-500">{org.reference || `VCS-${org.id}`}</span>
                                  ...
                                  <div className="flex items-center justify-between pt-3 border-t border-gray-100 mb-2">
                                    <div className="flex items-center gap-2">
                                      {org.assigned_to && org.assigned_to !== 'Unassigned' ? (
                                        <>
                                          <div className={`w-6 h-6 rounded-full ${getAvatarColor(org.assigned_to)} flex items-center justify-center text-xs font-semibold`}>
                                            {getInitials(org.assigned_to)}
                                          </div>
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl relative">
              ...
              <form onSubmit={handleSubmit} className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  {/* Custom Columns - Standardized UI */}
                  {customColumns.map(col => (
              ...
              <div className="p-6">
                ...
                <div className="mb-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-md text-sm text-slate-600 border border-slate-100 min-h-[80px]">
                    {viewingOrganisation.description || "No description provided."}
                  </div>
                </div>
                ...
                {customColumns.length > 0 && (
                  <div className="mt-4">
                    <div className="text-[10px] uppercase text-slate-400 font-bold tracking-wider mb-2">Custom Fields</div>
                    <div className="grid grid-cols-2 gap-y-4">
                      {customColumns.map(col => (
                        <DetailField key={col} label={col.replace(/_/g, ' ').toUpperCase()} value={viewingOrganisation[col]} />
                      ))}
                    </div>
                  </div>
                )}
