...
const SAMPLE = [
  { id: 1, title: "Passport & VISA Verification", reference: "PRL-2025-e5198a6e", description: "Operation work required as per inspection report.", priority: "Medium", status: "Completed", assignedTo: "ABC Maintenance", date: "2025-02-08", type: "Payroll" },
  { id: 2, title: "Resident Data Update", reference: "PRL-2025-c51690eb", description: "Operation work required as per inspection report.", priority: "Medium", status: "Pending", assignedTo: "Unassigned", date: "2025-09-26", type: "Payroll" },
  { id: 3, title: "AIRE Annual Reporting", reference: "PRL-2025-cda9bd4e", description: "Operation work required as per inspection report.", priority: "Low", status: "Completed", assignedTo: "In-house Team", date: "2025-06-02", type: "Payroll" },
  { id: 4, title: "Immigrant Status Validation", reference: "PRL-2025-2b9c6ef8", description: "Operation work required as per inspection report.", priority: "Urgent", status: "Completed", assignedTo: "Quick Fix Services", date: "2025-03-17", type: "Payroll" },
  { id: 5, title: "Address Verification Support", reference: "PRL-2025-f3c0c417", description: "Operation work required as per inspection report.", priority: "Low", status: "Pending", assignedTo: "Unassigned", date: "2024-12-19", type: "Payroll" },
  { id: 6, title: "Residency Renewal Assistance", reference: "PRL-2025-ca81318f", description: "Operation work required as per inspection report.", priority: "Medium", status: "Completed", assignedTo: "Quick Fix Services", date: "2024-10-24", type: "Payroll" },
  { id: 7, title: "Fix extractor fan", reference: "PRL-2025-576b7ce4", description: "Maintenance work required as per inspection report.", priority: "Low", status: "Pending", assignedTo: "Unassigned", date: "2025-04-24", type: "Payroll" },
];
...
// IMPROVED: Returns object with styles for badges
...
export default function Payroll() {
  ...
  const hasRead = canRead("payroll");
  const hasCreate = canCreate("payroll");
  const hasUpdate = canUpdate("payroll");
  const hasDelete = canDelete("payroll");
  ...
  const fetchHotels = useCallback(async (signal) => {
    try {
      ...
      const isCanceled =
        err && (err.name === "CanceledError" || err.code === "ERR_CANCELED" || axios.isCancel?.(err));
      if (!isCanceled) {
        console.error("Payroll fetchHotels error:", err);
        setHotels([]);
      }
    } finally {
  ...
  const loadTasks = useCallback(async (signal) => {
    setLoading(true);
    try {
      const res = await api.get("/api/payroll", { signal, params: { limit: 200 } });
      ...
      const formattedTasks = mapped.map((t) => ({
        id: t.id,
        title: t.title ?? "",
        reference: t.reference ?? `PRL-2025-${String(t.id).slice(0, 4)}`,
      ...
      console.error("Payroll loadTasks error:", err);
      setTasks(SAMPLE);
      setLoading(false);
    }
  }, []);
  ...
  async function handleCreateSubmit(e) {
    ...
    setCreating(true);
    try {
      const res = await api.post("/api/payroll", form);
      const newTask = {
        id: res.data.data.id,
        title: form.title,
  ...
  async function handleEditSubmit(e) {
    ...
    setEditing(true);
    try {
      await api.put(`/api/payroll/${editingId}`, form);
      const updatedTask = {
      ...
      <div className="bg-white border-b border-slate-200 px-8 py-6">
        <div className="max-w-[1500px] mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500 mt-1">
                <IconHome size={14} /> <span>/</span> <span>Employees</span> <span>/</span> <span className="text-orange-500 font-medium">Payroll</span>
              </div>
            </div>
            {hasCreate && (
              <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <KPICard title="Total Records" count={stats.total} color="blue" icon={<IconBuilding size={20} />} trend="+5 new" />
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="relative flex-1 sm:flex-none">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm w-full sm:w-72 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
                  />
                </div>
                <button className="p-2 border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors" title="Filter">
                  <IconFilter size={18} />
                </button>
                <button className="p-2 border border-slate-200 rounded-lg text-slate-600 hover:bg-slate-50 transition-colors" title="Export">
                  <IconUpload size={18} />
                </button>
              </div>
            </div>
                    ...
                    return (
                      ...
                      >
                        <td className="p-4 text-center align-top pt-5">
                          <input
                            type="checkbox"
                            className="rounded border-slate-300 accent-orange-500 w-4 h-4 cursor-pointer"
                          />
                        </td>
                        ...
                        <td className="p-4 align-top pt-5">
                          <div
                            className="font-semibold text-slate-800 text-base cursor-pointer hover:text-orange-600 transition-colors"
                  ...
                  <tr>
                    <td colSpan="8" className="p-12 text-center">
                        <div className="flex flex-col items-center justify-center text-slate-400">
                            ...
                            <p className="text-base font-medium text-slate-600">No payroll records found</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl relative border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <div>
                <h3 className="text-lg font-bold text-slate-800">Payroll Details</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl relative border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
              <div>
                <h3 className="text-xl font-bold text-slate-800">{showEdit ? "Edit Record" : "Add Payroll Record"}</h3>
                <p className="text-sm text-slate-500 mt-0.5">
                  Fill in the details below to track payroll.
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <form
              onSubmit={showEdit ? handleEditSubmit : handleCreateSubmit}
              className="p-6"
            >
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="e.g. Monthly Salary for January"
                            value={form.title}
              ...
              <div className="flex justify-end gap-3 mt-8 pt-5 border-t border-slate-100">
                ...
                >
                  {creating || editing ? "Saving..." : (showEdit ? "Update Record" : "Add Record")}
                </button>
              </div>
            </form>
          </div>
...
const IconSearch = ({ size, className }) => (
  <svg
    className={className}
    width={size}
    height={size}
...
const IconCheck = ({ size, strokeWidth = 2 }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const IconChevronDown = ({ size, className }) => (
  <svg
    className={className}
    width={size}
    height={size}
...
const IconCalendar = ({ size, className }) => (
  <svg
    className={className}
    width={size}
    height={size}
