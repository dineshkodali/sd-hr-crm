...
| **DEF-004** | **P1** | Security | Missing Content Security Policy (CSP) & HSTS Headers | HTTP request to API | Send GET to `/api/health` | **FIXED**: Installed custom middleware in `Backend/server.js:100` setting CSP, HSTS, X-Content-Type-Options & X-Frame-Options headers. | `Backend/server.js:100` |
