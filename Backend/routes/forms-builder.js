...
router.post('/tables/:tableName/columns', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const inputType = (input_type && String(input_type).trim()) ? String(input_type).trim().toLowerCase() : 'text';

    const inputTypeToSqlType = (t) => {
      const k = String(t || '').toLowerCase();
      if (k === 'text') return 'VARCHAR(255)';
      if (k === 'textarea') return 'TEXT';
      if (k === 'email') return 'VARCHAR(255)';
      if (k === 'number') return 'NUMERIC';
      if (k === 'date') return 'DATE';
      if (k === 'datetime' || k === 'datetime-local') return 'TIMESTAMP';
      if (k === 'dropdown' || k === 'select' || k === 'radio') return 'VARCHAR(255)';
      if (k === 'checkbox') return 'TEXT';
      if (k === 'file') return 'VARCHAR(500)';
      return 'TEXT';
    };

    const inferredType = inputTypeToSqlType(inputType);

    // Build ALTER TABLE query
    let columnDef = `${column_name} ${inferredType}`;
    ...
    // Add default value
    if (default_value) {
      if (inferredType.includes('CHAR') || inferredType === 'TEXT' || inferredType.includes('VARCHAR')) {
        columnDef += ` DEFAULT '${String(default_value).replace(/'/g, "''")}'`;
      } else {
      ...
      if (inferredType.includes('CHAR') || inferredType === 'TEXT' || inferredType.includes('VARCHAR')) {
        columnDef += ` DEFAULT ''`;
      } else if (inferredType.includes('NUMERIC') || inferredType.includes('INT') || inferredType.includes('DECIMAL') || inferredType.includes('REAL') || inferredType.includes('DOUBLE')) {
        columnDef += ` DEFAULT 0`;
      } else if (inferredType === 'DATE' || inferredType === 'TIMESTAMP') {
