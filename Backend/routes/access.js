...
// Permission rules (frontend/admin may set any combination):
// - If can_read is false => all actions will be false.
// - If can_read is true  => can_create/can_update/can_delete are applied as provided (any combination allowed).
router.post("/", protect, async (req, res) => {
  ...
  try {
    // Apply rules: if read is false, clear all actions; otherwise accept any combination
...
router.get("/me", protect, async (req, res) => {
  try {
    ...
    if (user.role === "admin") {
      // Return empty object - frontend will treat empty as full access for admins
    ...
    // Merge all permissions (use the highest permission for each module)
    const permissions = {};
    ...
    // Merge role permissions first (lowest priority)
    rolePermsQuery.rows.forEach(p => mergePermission(p.module, p));

    // Merge group permissions (medium priority)
    groupPermsQuery.rows.forEach(p => mergePermission(p.module, p));

    // Merge user permissions last (highest priority)
...
// Note: MODULES constant removed - using dynamic module list from frontend
