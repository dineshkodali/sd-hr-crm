...
// Get all branches (derived from hotels table)
router.get("/", protect, async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT DISTINCT branch as name, 
             NULL as id, 
             NULL as address, 
             NULL as manager_id, 
    ...
    // Normalize response to match expected frontend structure (id, name, etc.)
    // Since we don't have real IDs, we'll use the name as the ID for compatibility
    const branches = result.rows.map(row => ({
      ...row,
      id: row.name, // Use name as ID
      code: row.name.toUpperCase().slice(0, 3)
    }));
