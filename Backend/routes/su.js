...
async function getFormsBuilderMetaForTable(tableName) {
  try {
    ...
    if (!metaCheck.rows?.[0]?.exists) return new Map();
    ...
    return new Map((metaRes.rows || []).map((r) => [String(r.column_name), String(r.input_type || 'text')]));
  } catch (e) {
    return new Map();
  }
}

function coerceValueByDbType(value, dataType, inputType) {
  ...
  if (dt.includes('int') || dt.includes('numeric') || dt.includes('decimal') || dt.includes('double') || dt.includes('real')) {
    const n = Number(value);
    return Number.isFinite(n) ? n : null;
  }

  if (dt === 'date' || dt.includes('timestamp')) {
...
router.post("/users", protect, async (req, res) => {
  try {
    ...
    const suColsRows = await getColumnsForTable('service_users');
    const suCols = suColsRows.map((r) => r.column_name);
    const suColTypeByName = new Map(suColsRows.map((r) => [String(r.column_name), String(r.data_type || '')]));
    const suMetaInputType = await getFormsBuilderMetaForTable('service_users');
    ...
    const pushIfExists = (col, val) => {
      ...
      const coerced = coerceValueByDbType(v, suColTypeByName.get(col), suMetaInputType.get(col));
    ...
    // Convert array fields for PostgreSQL array columns
    // PostgreSQL expects arrays as JavaScript arrays or properly formatted array literals
    const arrToPgArray = (v) => {
      if (v === null || v === undefined) return null;
      if (Array.isArray(v)) {
        // Filter out empty strings and return array
        const filtered = v.filter(item => item !== null && item !== undefined && String(item).trim() !== "");
        return filtered.length > 0 ? filtered : null;
      }
      // If it's a string, check if it's comma-separated and convert to array
      if (typeof v === "string" && v.trim() !== "") {
        const split = v.split(",").map(s => s.trim()).filter(s => s !== "");
        return split.length > 0 ? split : null;
      }
      return null;
    };

    pushIfExists("vulnerabilities", arrToPgArray(body.vulnerabilities));
    pushIfExists("medical_conditions", arrToPgArray(body.medical_conditions));
    pushIfExists(
      "dietary_requirements",
      arrToPgArray(body.dietary_requirements)
    );

    // Dynamic/custom fields: include any request body keys that match existing DB columns
    // but exclude common system/core fields already handled above.
    ...
    const already = new Set(toInsert);
    for (const [key, rawVal] of Object.entries(body || {})) {
      if (!key || blocked.has(key) || already.has(key)) continue;
      if (!suCols.includes(key)) continue;
      // avoid overriding fields we explicitly normalize above
      if (key === 'dob' || key === 'date_of_birth') continue;
      if (key === 'admission_date') continue;
      if (key === 'property_id' || key === 'hotel_id' || key === 'accommodation_id') continue;
      if (key === 'room_id' || key === 'room_number' || key === 'room') continue;
      pushIfExists(key, rawVal);
    }
...
router.put("/users/:id", protect, async (req, res) => {
  try {
    ...
    const pushSet = (colName, val) => {
      ...
      const coerced = coerceValueByDbType(v, suColTypeByName.get(colName), suMetaInputType.get(colName));
      if (coerced === undefined) return;
    ...
    const handleArrayField = (value) => {
      ...
      if (typeof value === "string" && value.trim() !== "") {
        const split = value.split(",").map(s => s.trim()).filter(s => s !== "");
        return split.length > 0 ? split : null;
      }
      return null;
    };
    ...
    // Dynamic/custom fields from request body
    const blocked = new Set([
      'id',
      'created_at',
      'updated_at',
    ...
    for (const [key, rawVal] of Object.entries(req.body || {})) {
      ...
      // don't fight with normalized logic above
      if (key === 'dob' || key === 'date_of_birth') continue;
      if (key === 'admission_date') continue;
      if (key === 'property_id' || key === 'hotel_id' || key === 'accommodation_id' || key === 'property') continue;
      if (key === 'room_id' || key === 'room_number' || key === 'room') continue;
